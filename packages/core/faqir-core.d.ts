...
//     correct code does.
//
// Consumption. The runtime is a UMD file, so both shapes are typed here:
//
//     import Faqir from "@faqir-ui/core";        // bundler / node
...
export as namespace Faqir;
export = Faqir;

declare const Faqir: Faqir.FaqirGlobal;
...
declare namespace Faqir {
   ...
   *     }
   *
   * `export = Faqir` makes this namespace's members the module's exports, so an
   * augmentation names the interface directly — there is no `namespace Faqir`
   * wrapper. The same form opens {@link ControllerApis} and {@link Stores}.
   */
  interface PluginMagics {}
   ...
   */
  interface Scope extends Magics, PluginMagics {
    [key: string]: unknown;
  }
   ...
   */
  type MagicFactory = (el: Element, scope: Scope) => unknown;

  /** A function passed to `Faqir.plugin()`; it receives the engine itself. */
  type Plugin = (faqir: FaqirGlobal) => void;
   ...
   */
  interface SourceController<T = Record<string, unknown>> {
   ...
   */
  type ControllerFactory<A extends ControllerApi = ControllerApi> = (root: Element) => A;

  interface AccordionApi extends ControllerApi {
  ...
  interface DialogApi extends ControllerApi {
    open(): void;
    close(): void;
    toggle(): void;
  }

  interface BarcodeApi extends ControllerApi {
    render(): void;
    update(value: string): void;
  }

  /** A `range` calendar reports two dates; a single-date one reports a string. */
  type CalendarValue = string | null | { start: string | null; end: string | null };

  interface CalendarApi extends ControllerApi {
  ...
  interface CarouselApi extends ControllerApi {
    next(): void;
    prev(): void;
  ...
  interface ComboboxApi extends ControllerApi {
    open(): void;
    close(): void;
  ...
  interface CommandPaletteApi extends ControllerApi {
    open(): void;
    close(): void;
    filter(query: string): void;
  ...
  interface ContextMenuApi extends ControllerApi {
    open(x?: number, y?: number): void;
    close(options?: { restoreFocus?: boolean }): void;
  }

  interface DatePickerApi extends ControllerApi {
    open(): void;
    close(): void;
    getValue(): CalendarValue;
  ...
  interface DrawerApi extends ControllerApi {
    open(): void;
    close(): void;
    toggle(): void;
  }

  interface DropdownApi extends ControllerApi {
    open(options?: { focus?: string }): void;
    close(options?: { restoreFocus?: boolean }): void;
    toggle(): void;
  }

  interface FileUploadApi extends ControllerApi {
    getFiles(): FileList;
    open(): void;
  ...
  interface InputOtpApi extends ControllerApi {
    getValue(): string;
  ...
  interface MenubarApi extends ControllerApi {
    open(index?: number, focus?: string): void;
    close(restoreFocus?: boolean): void;
  }

  interface PaginationApi extends ControllerApi {
  ...
  interface PopoverApi extends ControllerApi {
    open(): void;
    close(): void;
    toggle(): void;
  }

  interface QrCodeApi extends ControllerApi {
    render(): void;
    update(value: string, errorCorrectionLevel?: string): void;
  }

  interface SelectCustomApi extends ControllerApi {
    open(): void;
    close(): void;
    toggle(): void;
  ...
  interface SheetApi extends ControllerApi {
    open(): void;
    close(): void;
    toggle(): void;
  }

  interface SidebarApi extends ControllerApi {
    toggle(): void;
    expand(): void;
    collapse(): void;
    open(): void;
  ...
  interface SliderApi extends ControllerApi {
  ...
  interface TableApi extends ControllerApi {
  ...
  interface TabsApi extends ControllerApi {
    activate(index: number): void;
  }

  interface TagInputApi extends ControllerApi {
    getValue(): string[];
  ...
  interface ToastApi extends ControllerApi {
    add(options?: {
      message?: string;
      tone?: string;
      icon?: string;
  ...
  interface ToggleGroupApi extends ControllerApi {
  ...
  interface TooltipApi extends ControllerApi {
    show(): void;
    hide(): void;
  }

  interface TreeViewApi extends ControllerApi {
   ...
   */
  interface Inspection<A extends ControllerApi = ControllerApi> {
    el: Element;
...
declare global {
  interface Window {
    ...
    Faqir?: Faqir.FaqirGlobal;
    /** Installed by both engine builds on every Faqir page. */
    __FAQIR_DEVTOOLS__?: Faqir.Devtools;
  }
}
