...
| `faqir_theme_info` | `{ theme? }` | With no argument: a summary card for every registry theme — mood, scheme, dark mode, pairings, `kind` (`authored` / `generated` / `companion`), the fourteen character `axes` derived from its stylesheet, and its `distinctiveness` from the nearest other theme. With `theme`: that theme's full manifest, including its `seed` (on a generated theme) and its overridden/inherited token sets. `active_theme` reflects the host project's configured theme when run inside one. |
| `faqir_theme_list` | `{ axes?, mood?, kind? }` | Themes chosen **by axes** rather than by adjective: `axes` maps dotted paths to required values (`{ "depth": "glass" }`, `{ "type.pairing": "serif-editorial", "density": "spacious" }`); every clause must hold, and a print companion (no axes) matches none. Unknown paths and out-of-vocabulary values error with the options listed — an empty result means no shipped theme is like that. Returns summaries with their `axes`, plus `total`. |
