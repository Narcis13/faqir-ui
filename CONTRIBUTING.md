...
site/                   The documentation site — a Faqir project
  ...
  content/home.html     The only hand-written page content on the site
  dist/                 GENERATED by `bun run build:docs` (git-ignored)

tests/                  Test files mirroring src/ structure
  fixtures/             HTML fixtures for audit testing
```

## The documentation site (`bun run build:docs`)
...
reference, a navigation shell and a live example per component. **Adding a
component to `registry/` adds its page with zero edits to `site/`** — if you find
yourself editing `site/` to describe a component, put it in the manifest instead.

The site is built from registry components and design tokens only, and its own
pages are held to `faqir audit` (zero findings, every severity) and axe (zero
violations, both colour schemes). Full reference: [`docs/docs-site.md`](docs/docs-site.md).
