...
27 themes ship with the registry. A theme redefines tokens only — no component markup changes, so switching one never invalidates a page. Pick by **mood**, then `faqir theme set <name>`.
...
| `clinical` | clinical, medical, calm, legible, healthcare | both | native | `slate`, `document` |
...
| `fintech` | fintech, dashboard, dense, dark, data | both | native | `midnight`, `slate` |
...
| `ink` | letterpress, monochrome, sepia, slab, print | light | none | `ink-document`, `paper` |
...
| `neumorph` | neumorphic, soft-ui, pressed, tactile, rounded | both | native | `soft`, `candy` |
| `nordic` | nordic, airy, cool, geometric, spacious | both | native | `glass`, `slate` |
...
| `sunset` | sunset, warm, gradient, friendly, consumer | both | native | `organic`, `aurora` |
