...
//   scripts/gallery.js                theme-gallery wiring (authored, site/lib/)
//
...
//     `dashboard-shell` nav (home, index, component, token, playground, gallery),
...
//     by design.
//
// The site ships JavaScript on exactly three pages (task 0.7-14): the playground
// (the audit engine, compiled for the browser, plus the manifests it audits
// against) and the theme gallery + its frame. Everything else is still static
// HTML + one stylesheet. Nothing on the site fetches: the audit runs in the page.
//
...
${(input.scripts ?? []).map((src) => `<script src="${u(src)}" defer></script>`).join("\n")}
...
function renderHomePage(ctx: {
  ...
  const stats =
    ...
    )} design tokens</a> and <a data-ui="link" href="${escAttr(THEMES_PAGE)}">${esc(
      String(ctx.themeCount),
    )} themes</a>. ` +
    `Every page you are reading was generated from the same manifests an agent reads — ` +
    `including the <a data-ui="link" href="${escAttr(
      PLAYGROUND_PAGE,
    )}">audit playground</a>, which runs the framework's own audit engine in your browser.</p>`;
  ...
  const themeButtons = themes
    .map(
      (t) =>
        ...
        `data-theme-pick="${escAttr(t.name)}" aria-pressed="${t.name === ctx.config.theme}">${esc(
          t.name,
        )}</button>`,
    )
    .join("\n");
  ...
  const cards = themes
    .map((t) => {
      ...
      return (
        ...
        )}" loading="lazy" data-theme-frame="${escAttr(t.name)}" ` +
        `style="inline-size: 100%; block-size: 22rem; border: 1px solid var(--color-border); ` +
        `border-radius: var(--radius-md); background: var(--color-bg);"></iframe>\n` +
        `          </div>\n` +
...
/**
 * The site stylesheet: tokens, base, then every component stylesheet — the
 * cascade order `faqir init` installs, minus the theme. One file, one `<link>`,
 * so a page works from any directory depth and any static host.
 *
 ...
 */
export function buildSiteStylesheet(registryRoot: string): string {
