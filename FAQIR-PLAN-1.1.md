...
**Acceptance criteria**
- [ ] Seven role/voice tokens exist and every heading, body and control surface in the registry consumes them; direct `--font-sans`/`--font-serif` use is zero and gated.
- [ ] No visual change at defaults: computed `font-family` strings on a control and a heading are byte-equal before/after (asserted), and no theme's `tokens_overridden` changed (`gen:theme-manifests` produces a diff only in `tokens_inherited`).
- [ ] `@ui:tokens` headers and `tokens_used` updated on every touched component; `audit:registry`, `check:skill`, `check:core-package` green.
