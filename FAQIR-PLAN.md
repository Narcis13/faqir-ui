...
**Acceptance criteria**
- [x] Zero undeclared attributes on grid; forced collapse gone (unsuffixed `data-cols` holds at all widths; collapsing is an explicit authoring choice). (`tests/primitives/grid.test.ts` — 28 tests over the parsed sheet via the new shared resolver `tests/helpers/css-cascade.ts`: completeness BOTH ways — nothing selected-but-undeclared, nothing declared-but-unruled, base and all four tiers — and the only value-less presence selector left is the declared boolean `data-scroll`; the 1.x forced collapse rode bare `[data-cols]`/`[data-span]` selectors, so that assertion is the structural regression. `data-cols="4"` resolves `repeat(4, 1fr)` at 390px, the span rules are five unconditioned child rules no media block can crush, and 640.5px lands in the sm tier — the 1.x range pair's dead zone is a named test. Tier precedence is encoded in SPECIFICITY (each tier block repeats `[data-ui="grid"]` once more than the tier below: base (0,2,0) → xl (0,6,0)), asserted structurally per selector AND behaviorally by resolving five probe ladders identically against the shipped, fully reversed, and selector-sorted rule orders — the source-order independence the task demanded, which 0.8-03's stack deliberately did not have.)
- [x] `auto` mode shipped, in the manifest, on the reference page. (`data-cols="auto"` compiles to `repeat(auto-fit, minmax(min(100%, var(--grid-min)), 1fr))`; `data-min` is an enum prop over rem steps 8|12|16|20|24 (description disambiguates rem from gap's spacing steps) whose rules only re-declare `--grid-min` — the default sits on the base `[data-ui="grid"]` rule at (0,1,0), so the override is specificity-safe at any source position. `auto` is a full member of the cols value set, so every tier accepts it (`data-cols-md="auto"` tested). The media-free promise is asserted the strong way: at every probe width, the WINNING rule for both `grid-template-columns` and `--grid-min` has `media: null` (resolveRule), not merely "no @media near the text". grid.html now leads with two intrinsic examples, and the manifest's `html_auto` template + composition notes tell agents to reach for it first, per §15 doctrine 1.)
- [x] Changelog records the mobile-first inversion; every pattern consuming grid vocabulary updated in the same commit. (grid.manifest.json is 2.0.0 with a `breaking: true` entry covering the inversion, the range→ladder retier of `data-cols-{sm,md}`, the removed collapse/crush, the newly declared gap 0–16 (default honestly 4→0: no bare-grid gap rule ever existed), span/scroll, and both migration recipes. Every `data-ui="grid"` placement updated: stats-dashboard's two KPI rows (`data-cols="1" data-cols-md="2" data-cols-lg="4"` and `data-cols="1" data-cols-md="3"`, manifest template + slot description + sheet comment too), dashboard-shell's two content grids, stat.html's example row — all render identically at the capture viewport, so visual baselines survive. stats-dashboard's suite gained a cascade test resolving the authored metrics attributes through grid.css itself: 1fr at 390 and 640.5, `repeat(2, 1fr)` at md, `repeat(4, 1fr)` at lg — coverage grew 25→26, nothing shrunk. Patterns that borrowed only the *attribute names* for their own vocabulary (pricing, feature-grid, key-value) are 0.8-09's sweep by that task's own text. `data-scroll` kept its 0.x phone-strip meaning, re-expressed mobile-first: a snap strip below sm, restored to a grid from sm up by the boosted tier rules. Bindings regenerated (LGrid: `auto` in the cols union, colsSm…Xl/gapSm…Xl reusing the base unions, `scroll?: boolean`), skill + registry-index regenerated, codegen snapshots updated. bun test 2839 pass / 0 fail (+29); typecheck, audit:registry (84 pages), check:bindings, check:skill, check:registry-index, check:schema-refs, check:audit-browser, check:docs (291 files) all green.)
...
**Acceptance criteria**
- [x] Both primitives audit-clean, axe-clean, in the visual matrix, installable via `faqir add`. (Full three-file anatomy each with the `<!-- @ui:component -->` header, so both entered every matrix by *discovery* and no suite was edited to admit them: `audit:registry` now scans 86 component pages (was 84) for zero findings, the docs-site suite's `auditHtmlSource` pass over every generated page and live example stays at zero at any severity, and `playwright --config=playwright.a11y.config.ts --grep "cluster|switcher"` is **12/12 green** — the four axe cases each ({default, contrast} × {light, dark}) plus both docs pages in both schemes. The visual matrix grew by 48 cases each (12 themes × 2 schemes × 2 directions); baselines are git-ignored and reseeded on `main` by CI (tests/visual/README.md), so there is nothing to commit for them. `faqir init && faqir add cluster switcher` in a scratch project writes six files and rebuilds the bundle (138 matching selectors). Both are **born clean** under the rule 0.8-10 will generalize: `tests/primitives/cluster.test.ts` (26 tests) and `switcher.test.ts` (22) assert completeness BOTH ways — nothing selected-but-undeclared, nothing declared-but-unruled, base and every tier — with `tokens_used` derived from each sheet's own `var()` reads and the `@ui:tokens` header asserted identical to it. cluster additionally proves it is direction-agnostic by running the framework's own `buildLogicalPropertyResults` over the sheet for zero findings *and* by resolving the push idiom to `margin-inline-start: auto` on a child (the "actions on the right" idiom that is the far end in RTL), with a regex sweep banning every physical spelling; its four tier blocks repeat `[data-ui="cluster"]` once more than the tier below (base (0,2,0) → xl (0,6,0)), so five probe ladders resolve identically against the shipped, reversed and selector-sorted rule orders — grid 2.0's source-order independence, adopted at birth.)
- [x] switcher is container-driven only — no viewport media query in the file. (Asserted three ways, because the claim is the component: **structurally** — `switcher.css` contains no `@media` and no `max-width` at all, every parsed rule has `media: null`, and no attribute in the sheet parses as tier-suffixed (`data-gap` is deliberately *not* responsive: a `data-gap-md` would re-couple to the viewport); **by prelude** — the four `@container` conditions are rebuilt from the canon module (`containerQuery(tier, "faqir-switcher")`) and compared, so the thresholds cannot drift from the ladder; **behaviourally** — the flip is resolved through the real rules at mocked *container* widths and every case is repeated at four viewports from 390px to 1920px, where a viewport that changes nothing is what zero coupling means. The mechanism needed the shared cascade helper to grow a second axis (`tests/helpers/css-cascade.ts`: `@container` blocks are collected with their prelude, and a resolution can target a CHILD of the component — `resolveIn`/`resolveValue` over a `ResolveContext`, with `:not()` understood as a negated condition of the same weight; `resolve`/`resolveRule` keep their signatures and grid/stack/stats-dashboard resolve byte-identically). The flip lives on the children because `@container` styles a container's DESCENDANTS, never the container itself — so a row that measures *itself* cannot be `flex-direction` on the root: `flex-basis: 100%` is the column, the container rules release it to `0` for equal-width columns, at (0,2,0) against the base's (0,1,0) so re-ordering the sheet changes nothing. **The in-session decision on the threshold steps: they are the canon tiers, not aliases of 0.8-06's measure tokens.** A container prelude cannot read a custom property — the same reason the canon is a module and not a token — so aliasing `--measure-*` would have duplicated their numbers under a second set of names and re-created exactly the multi-ladder drift 0.8-01 killed (`table`'s 30/48/64rem, where `sm` meant 480px). `data-threshold="sm|md|lg|xl"` therefore names 40/48/64/80rem measured on the component's own inline size; the declared default `sm` is made *real* rather than documented by `:not([data-threshold])` sharing the sm block at equal weight, so a bare `<div data-ui="switcher">` flips at 40rem of its own width. `inline-size: 100%` on the root is load-bearing, not decoration: inline-size containment means the box may not take its width from its contents, so an unsized container in a shrink-to-fit parent resolves to zero and every threshold would read "narrow".)
- [x] Docs site's wrap use cases migrate to cluster where semantically right (else stack `data-wrap`). (Four call sites, all of them genuine cluster: the button row and the swatch row in each of the twelve theme-preview frames — an inline `flex-wrap` escape until 0.7-20 filed it, `stack` + `data-wrap` when 0.8-03 declared it, and now the component whose wrap is intrinsic rather than opted into — and the theme and colour-scheme switchers on the gallery page, which additionally shed the deprecated `data-variant="horizontal"` spelling and gain a wrap they never had. The assertion moved with the markup: stack's suite keeps the claim that is stack's (no `flex-wrap` escape, no inline `display: flex`, in any frame) while cluster's suite owns the positive one — every frame carries `<div data-ui="cluster" data-gap="2">` and `…data-gap="1">` and no `data-wrap`, and `themes/index.html` carries both labelled groups with no deprecated spelling left. `hero`'s bespoke actions CSS and the six patterns with local `flex-wrap: wrap` are deliberately untouched: they are 0.8-08/0.8-09's sweeps by those tasks' own text, and changing pattern markup here would have moved their visual baselines for no test in this task's scope.)
...
**Acceptance criteria**
- [x] `container` shipped with full anatomy, manifest, tests, docs page — enters all matrices. (`registry/primitives/container/{css,html,manifest}` + `tests/primitives/container.test.ts` (36 tests), born clean under 0.8-10's coming rule: completeness is asserted BOTH ways — nothing selected-but-undeclared, nothing declared-but-unruled, base and all four tiers — and the sheet selects on **zero** value-less attributes, so there is no boolean surface to drift. `data-measure` is a responsive variant group with tier precedence encoded in SPECIFICITY at birth (base (0,2,0) → xl (0,6,0)), proved behaviourally by resolving four probe ladders identically against the shipped, fully reversed and selector-sorted rule orders — grid 2.0's mechanism (0.8-04) adopted, not re-earned. `data-gutter` is an enum **prop** over spacing steps 0–16 and is deliberately NOT responsive (asserted: no `data-gutter-<tier>` is selected anywhere), because a gutter restates what the padding parent already says; the declared default of 0 is made real — a bare container resolves no `padding-inline` at all, which is what lets it nest inside the docs shell without doubling its inset. The naming collision the task flagged is handled where the framework can enforce it: the manifest description disambiguates `container` from `container-type`, and the suite asserts the sheet contains neither `container-type` nor `@container` — a container is a plain block box, and `switcher` (0.8-05) remains the only component that measures itself. Enters every matrix generically: `LContainer` in both bindings (one exported `LContainerMeasure` union reused by all four tier props, not five types), the docs page with its Responsive column, the skill's primitives reference, `registry-index.json` (85 → 86 components) and the site (297 → 300 files).)
- [x] Measure ladder tokenized once; surface/form-page/wizard consume it. (Four tokens in `aliases.css` — `--measure-narrow: 32rem`, `--measure-content: 48rem`, `--measure-wide: 72rem`, `--measure-prose: 65ch` — and a drift test that reads all four consumers (`container`, `surface`, `form-page`, `wizard`) and asserts each references the ladder, that every referenced token resolves against `aliases.css` (which is exactly what `token-exists` checks; `audit:registry` is clean), and that **no** hardcoded measure width survives in any of them: `640px`, `768px`, `1024px`, `1280px`, `1400px`, `32rem`, `72rem` are all absent. **The reconciliation decision, since the task left it open: surface's `data-max` values are re-based, not preserved** — and surface goes to 2.0.0 with a `breaking: true` entry carrying the migration. The 1.x ladder was five hardcoded px widths named `sm|md|lg|xl|2xl`, i.e. the breakpoint canon's own names used for max-widths, so `data-max="lg"` read as "at the lg tier" when it meant "1024px wide" — precisely the multi-ladder drift 0.8-01 killed and 0.8-05 refused to re-create. Preserving those five values would have required a second token ladder under a second set of names; instead `data-max` now speaks the same five-value vocabulary as container's `data-measure`, resolving to the same tokens, and the suite asserts both halves — each value resolves to its token, and each retired canon-named value (`sm`…`2xl`) now selects nothing. The change costs nothing to reconcile because `[data-ui="surface"][data-max]` had **zero consumers** in the repo (measured: no HTML anywhere writes it), so no markup moved and no visual baseline shifted. A separate token test asserts `--measure-<tier>` may never exist for any canon tier, so the two ladders cannot converge again by accident. Both `data-max` and `data-align-text` were in surface's CSS but absent from its manifest — the same undeclared-attribute gap 0.7-20 found on stack — and are now declared props, so the bindings, docs, skill and context know they exist. form-page and wizard are re-based on `var(--measure-narrow)` with markup unchanged, both manifests' `tokens_used` updated.)
- [x] The 72rem inline-style escape is gone from every generated page. (`src/generator/docs.ts` swaps `<article data-ui="prose" style="max-width: 72rem;">` for `<div data-ui="container" data-measure="wide"><article data-ui="prose">` — one edit, 94 pages. Verified three ways rather than asserted once: the suite scans **every** emitted `.html` for a `style` attribute containing `max-width` and finds none anywhere on the site; every page built through the shared docs shell (selected on the shell's own `aria-label="Documentation"` nav, not on `data-ui="dashboard-shell"` — the pattern's verbatim example pages contain that string without ever going through the shell) carries the container wrapper, a bare `<article data-ui="prose">` and no `72rem`; and the bundled `styles/faqir.css` is checked to actually contain container's rules and `--measure-wide`, since deleting an escape without shipping its replacement would otherwise pass. On the built site both `style="max-width` and `max-width: 72rem` are now at **zero occurrences in all 300 files** — the one remaining hit after the first build was a comment in `container.css` quoting the escape it replaces, reworded so even a naive grep over shipped bytes confirms the claim. Two behaviour changes worth recording: the cap is now logical (`max-inline-size`), so it flips under RTL where the physical `max-width` did not, and the prose column is now genuinely centred (`margin-inline: auto`) rather than left-aligned in its flex parent — centring is what "centred measure column" means, so it is the point of the swap, not a side effect. **Scope note:** the task named two escape sites (`docs.ts:683`, `:1458`); only the first is a measure escape. The line the plan pointed at as `:1458` is the theme-preview frame's `<main style="padding: …; display: grid; gap: …">`, which is a padding/grid escape whose honest replacement is `grid` plus a gutter, not `container` — swapping it would move twelve theme frames' visual baselines for no test in this task. Filed as follow-up **0.8-13** rather than silently absorbed.)
...
**Acceptance criteria**
- [x] Scale extended and rhythm aliases shipped, documented in the token reference page (docs site regenerates). (`--space-{32,40,48,64}` = 8/10/12/16rem in `spacing.css`, and four rhythm aliases in `aliases.css` — `--section-gap-{sm,md,lg}` over `--space-{12,20,32}` and `--content-gutter` over `--space-6`. The aliases are **composed, not declared**: each is `var(--space-N)` and nothing else, which is what makes them free for density (a `var()` in an alias re-substitutes in whatever scope re-declares the step) — asserted both ways, that each resolves to its step's value and that no rhythm alias mentions `--density-scale` in *either* the base or the compact block. `--section-gap-lg` is asserted to exceed the old `--space-24` ceiling, which is the whole reason the scale was extended. `tests/tokens/spacing.test.ts` (11 tests) also pins the scale's own arithmetic — every step a whole number of 4px units, and the ladder strictly ascending with 64 as the top, derived from `spacing.css` rather than listed. Documentation is derived, not written: the same test drives `parseTokenReference`/`tokenAnchor` from `src/generator/docs.ts` and asserts all eight land with the right group (`spacing` / `aliases`), verbatim value and anchor, so the token-reference page cannot silently omit them; `check:docs` is green at 300 files.)
- [x] Density remaps the new steps; nesting/reset proven at the new sizes. (Both blocks re-declare all four steps — `calc(<rem> * var(--density-scale))` in compact, verbatim in comfortable — per the file's own doctrine, and the new aliases are re-declared beside them. The **0.7-11 probe-rule method** is what proves it: happy-dom substitutes `var()` but does not evaluate `calc()`, so a probe rule per token consumes it through a real `padding` and the computed value is read back at base density and inside a compact scope — each new step resolves to its rem value and to `× 0.75`, each rhythm alias likewise, and `--space-0`/`--space-px` are invariant in *computed text*, not merely in number. **The drift test was verified, not extended**: `density.test.ts`'s `readsScaled` derivation over `aliases.css` + `doc-aliases.css` picked the four new aliases up with zero edits to it, and a new test asserts exactly that — the derived set *contains* `--section-gap-*`/`--content-gutter` — so replacing the derivation with a literal list would still pass the old assertion and fail this one. Its sibling asserts the same for the four new steps through `SCALED_SPACE`, itself derived from `spacing.css`. Nesting and reset need no new cases: they already run against `--space-4`, and the completeness tests prove the new steps are in the same derived set the nesting cases traverse. Twelve theme manifests regenerated — `tokens_inherited` grew by 8 apiece — and the core CDN bundle re-hashed.)
- [x] Zero dangling `var(--…)` references registry-wide, enforced by a test that stays. (**The rule gap, answered:** `token-exists` is a *project* rule — `checkTokens` walks `<outputDir>/{primitives,recipes,patterns}` from `.faqir/config.json`'s installed list — and `scripts/registry-audit.mjs` ran three gates, none about tokens. The registry was never an input, so a project installing `settings-page` would have been warned and the framework's own source of truth never was. All three hypotheses are pinned as tests: the extractor *does* see `var(--x, fallback)` (refuted), none of the three documented skips match `space-48` (refuted), and `checkTokens`' own source contains `installed.primitives` + `join(outputDir,` and the string `registry` nowhere (confirmed). Fixed by `findDanglingTokenReferences` in `src/audit/checker.ts` + **Gate 4** of the registry audit, sweeping 101 stylesheets against 298 tokens. The predicate is derived, with no allowlist: a reference passes if it names a design token, or a knob the same stylesheet declares (`--grid-min`, `--sidebar-width`, `--icon`), or an author/runtime knob carrying a fallback whose namespace is outside the token vocabulary (`--shell-sidebar-width`, `--pos`) — and a fallback deliberately does *not* excuse a name inside a family the token layer defines, because such a name reads as a token to every agent and human. That found two more of the same species beyond the one the task named: `--text-tabular-width` (reads as the `--text-*` type scale) and `--z-tooltip` (reads as the `--z-*` layer ladder), both undeclared knobs wearing a fallback. Both are now declared on their component root — sidebar's shipped pattern — which is behaviour-identical (`--z-tooltip` always resolved to `--z-dropdown`) and makes them real, discoverable, overridable. `settings-page.css:137` drops its fallback for the real token; its `@ui:tokens` header and manifest `tokens_used` gained `space-48`. `tests/audit/token-resolution.test.ts` (17 tests) keeps the sweep at zero, guards the vacuous pass by asserting it is clearing 1000+ real references against 200+ tokens, asserts the gate script is actually wired to the same function, and exercises each accept/reject arm of the predicate directly.)
...
**Acceptance criteria**
- [x] Zero non-canon width preludes under primitives/recipes; all min-width form. (`tests/responsive-canon.test.ts` walks all **71** sheets under `registry/primitives` + `registry/recipes` and parses their **78** preludes, of which **25** carry a width. The predicate is deliberately shaped for 0.8-10 to lift verbatim: a prelude that *mentions* width must be exactly one parenthesised canon `min-width` condition (optionally preceded by a container name), and the canon set is built from `BREAKPOINTS`/`minWidth`, never re-typed; everything without a width — `prefers-reduced-motion`, `prefers-color-scheme`, `print` — is exempt by construction rather than by an allowlist anybody has to maintain. Three offenders existed and are gone: `input` (`max-width: 640px`), `stepper` (`max-width: 640px`) and `table`'s three container ceilings. `surface` needed nothing — 0.8-06 had already left it with no `@media`/`@container` at all, which the sweep now proves rather than assumes. A companion assertion pins the *unit*: every width literal in a prelude is `rem`, never `px`, and its number is one of 40/48/64/80. Two guards keep the section from passing vacuously (sheet and prelude counts) — the first of which caught a wrong initial threshold and is the reason the numbers above are measured, not estimated.)
- [x] Table CSS + JS + manifest agree on one ladder; changelog records any threshold shifts. (**One ladder, four artifacts.** CSS: the three `@container faqir-table (max-width: 30/48/64rem)` ceilings became `min-width` floors rebuilt from `containerQuery(tier, "faqir-table")` and compared to the module, so they cannot drift. JS: `STACK_BREAKPOINTS` is `{sm: 640, md: 768, lg: 1024}` and the suite parses that literal out of **five** files — the recipe, both engine builds, and the vendored react/vue controllers — comparing each to `BREAKPOINTS[t].px`. It is a literal and not an import because `scripts/build-core.mjs` strips every `import` line when it inlines a controller into the UMD closure (verified in the script, not assumed), so "imported truth" is impossible by construction; the equality is enforced from outside instead, which is what "no second source of truth" can actually mean here — the comment in `table.js` says so and names the test. Manifest: `data-stack-below` and `data-hide-below` are now **declared** props (they were prose in a slot description before), each an `enum` over exactly the canon tier names, asserted against `TIERS`. **The shift:** only `sm` moves — 30rem/480px → 40rem/640px — so `table` goes to **3.0.0** with `breaking: true` carrying the migration (`data-stack-below="480"` for the old value; `data-hide-below` has no equivalent by design), feeding 1.0-03. **The CSS inversion is the interesting half**, because "hide below X" cannot be spelled mobile-first without changing what the base says: the base now *hides* a prioritised column and each tier *reveals* it with `display: revert`, which lets the element's own tag supply its display instead of hardcoding `table-cell`. Three specificity facts make that behaviour-preserving, and each is a test rather than a comment: the base hide carries `[data-responsive]` (0,3,0) since nothing else scopes it — a plain table has no `container-name`, so a table without `data-responsive` still hides nothing, asserted both ways; each reveal is (0,3,0) and sits *before* the stacked-mode block, so stacked mode still wins and card rows keep every column, exactly as 1.x behaved; and `:not([data-col-hidden])` stops a widening container from revealing a column the controller hid — the one interaction the inversion would otherwise have introduced. `input` (1.1.0) and `stepper` (1.1.0) get changelog notes rather than `breaking`: their threshold is *unchanged* at 640px = 40rem, and only the boundary itself moves to the wide side, which is inherent to max→min and is asserted at `sm.px - 1` / `sm.px` in both suites. `input`'s `data-width` was also undeclared in any manifest — the 0.7-20 species again, found while rewriting its one rule — and is now a declared prop; per `src/bindings/ir.ts` only *boolean* props with an `attr` reach the bindings, so this is an agent-surface declaration exactly like `surface`'s `data-max` in 0.8-06, and `check:bindings` reports zero drift. The shared cascade helper grew `resolveDeep` for this task: `stepper` and `table` style *descendants*, which `resolve`/`resolveIn` cannot reach, so the new entry point matches an explicit ancestor chain and sums specificity across every compound — added alongside the existing functions rather than by widening them, so grid/stack/switcher/cluster/container all resolve byte-identically.)
- [x] Size gate green with the number stated; visual matrix unchanged elsewhere. (**Byte-neutral, exactly — not "± noise".** `engine + controllers` is **43.07 KB gzip ≤ 44.00 KB**, and minifying the pre- and post-change `faqir-core.js` through the same `bun build --minify` pipeline gives **146,676 bytes minified / 43,992 bytes gzip for both**: a zero-byte delta, which is what swapping `480` for `640` and adding comments the minifier strips should cost. All eight size targets pass. **Visual matrix:** rather than reason about it, `bun run test:visual -g "(table|input|stepper)__default"` was run — and it fails six cases. All six were then reproduced on a **stashed, clean tree**, so they are pre-existing baseline rot, not this task's: `recipe__table` is off by page *height* (baseline 1280×1449, render 1280×3042) and `primitive__input__dark` by 2677 px. Filed as follow-up **0.8-14** with the evidence, including the wider finding that only the `default` theme is baselined at all, so the visual suite has never been green as a whole. This task's own changes cannot move the matrix, and the reason is structural rather than hopeful: the capture viewport is 1280px, above `sm`, so `input`'s cap and `stepper`'s labels render identically on both sides; and no fixture anywhere under `registry/**` or `tests/visual/matrix.ts` writes `data-responsive`, `data-hide-below`, `data-stack-below` or `data-width` — the one `data-hide-below="md"` table in `table.html` sits on a root *without* `data-responsive`, so no container is established and nothing hides, before or after. `audit:registry` clean on all four gates (112 logical-properties sheets, 12 themes, 87 markup pages, 101 sheets against 298 tokens); `typecheck` and all seven `check:*` gates green.)
...
**Acceptance criteria**
- [x] Both rules shipped in CLI + browser bundles with parity proven. (`src/audit/css-rules.ts` — two pure functions over (stylesheet, manifest), reading `src/utils/breakpoints.ts` for the canon and the manifest for the declarations, with `findSelectedAttributes`/`findAtRulePreludes` added to `src/parser/css-parser.ts` as the shared scanners. **They are `CSS_RULES`, not `ALL_RULES`, and the divergence from the task text is deliberate**: `ALL_RULES` is typed `check(component: ParsedComponent, manifest: Manifest)` over parsed *markup*, so putting a stylesheet rule in it would mean handing every HTML rule a CSS source to ignore. `CSS_RULES` is the same shape the `logical-properties`/`no-important` family already uses, and one export feeds all four consumers: `getRuleInventory()` (so `faqir audit --rules` lists them — verified in a scratch project), `checkCssAntiPatterns` in the CLI, gate 5/6 of `audit:registry`, and `FaqirAudit.auditComponentCss` in the browser. The bundle rebuilt as predicted (35.71 KB raw / 12.28 KB gzip, `check:audit-browser` green) and the rules ride the *same* functions rather than a second implementation, which is what makes parity structural. Parity is proven on **92 stylesheet fixtures** — all 86 registry component sheets paired with their own manifests, plus six seeded/degenerate sheets — each audited by the committed bundle in a bare `node:vm` context and deep-compared to the in-process functions (0.7-14's method; there is no Chromium leg in that method — the bundle is exercised in `node:vm` and, for the page wiring, happy-dom). The seeded six are the point: 86 clean sheets would agree on the empty array 86 times and prove nothing about the finding path. `skipRules`, the default file label and the never-throws promise are asserted through the bundle too.)
- [x] `audit:registry` includes them at zero findings; a seeded violation of each fails CI. (Gates 5 and 6, pairing each of the **86** manifests with the sheet its `files.css` names — so `icon`, whose stylesheet is `icons.css`, is checked rather than silently skipped, which the naive name-based pairing did. Getting to zero cost the real work of this task: the rule found **40 undeclared attributes across 12 components**, four times the ten layout had shipped, and every one is now declared — `button` `data-full`; `text` `data-truncate`/`data-tabular`/`data-state="done"`; `empty-state` and `separator` `data-size`, both *protocol* attributes styled by CSS that no manifest named; `combobox`/`select-custom`/`tag-input`/`command-palette` `data-highlighted`+`data-hidden`; `calendar` `data-today`/`data-outside`; `input-otp` `data-filled`/`data-active`/`data-caret`; `toggle-group` `data-state` (it declared no states at all); and `table` with **twenty**. The split is 0.7-20's own decision applied generally: **controller-written → state, authored → prop** — table's twenty divide 17/3 exactly along the line its controller's header comment already drew, and its `data-collapsed` is declared as `row-collapsed` because the group-header's `data-state="collapsed"` already holds that name. `empty-state`/`separator` take `"default": ""` (the `document`/`stat` precedent) because absence is a real value there, not a synonym for a middle size. Twelve manifests get a minor bump and a changelog entry, all `breaking: false` — no CSS, controller or markup moved, so no visual baseline moves either. The payoff is the generated surfaces: `LButton` gains `full`, `LText` gains `done`/`truncate`/`tabular`, `LEmptyState`/`LSeparator` gain a `size` union — `check:bindings` reports zero drift after regeneration, and `check:skill`/`check:registry-index`/`check:docs` are green. Seeded violations are a test, not a claim: `tests/audit/css-rules.test.ts` takes real components and (a) deletes `stacked` from table's manifest → the eleven rules that select it report as one finding, (b) appends `[data-dense]` to grid's sheet → one finding, and asserts the unseeded sheet stays silent, (c) flips grid's `md` floor to `max-width` and (d) nudges its `lg` floor to 65rem → one finding each. End-to-end, a seeded `[data-elevated]` + `@media (max-width: 40rem)` in a scratch project's `ui/primitives/button/button.css` makes `faqir audit` report both and exit non-passing. 34 rule tests + 7 parity tests; suite 3037 pass / 0 fail (+38).)
- [x] Follow-up 0.7-20's "general gap" clause resolved (edit the row per house rule). (Row edited in place and flipped to ✅ with the measured result — the direction that made it work is that the rule starts from what the CSS *selects on*, which is the one direction no existing rule could look: `valid-variant`/`valid-size`/`valid-state` all validate the values of attributes a manifest already declares, so an attribute it never declares is invisible to every one of them. `tests/primitives/stack.test.ts` keeps its per-component assertions, now backstopped rather than load-bearing. One gap is deliberately NOT closed here and is filed as **0.8-15**: value-level declarations. A sweep found 21 selected values across 4 components outside their group's `values`, and three of the four are not typos but two different axes sharing one attribute in one sheet (`toast`'s `data-variant` is a position axis styled with severity values; `document`'s `data-format` is a page size styled with number formats), so each needs a modelling decision before any rule can gate it — an hour of remediation would have been a guess, and this task's rule is attribute-level by its own text.)
...
**Acceptance criteria**
- [x] Viewport axis live in CI with the snapshot count stated; sharding updated if needed. (`tests/visual/responsive.pw.ts` sits inside the existing `playwright.config.ts` `testMatch`, so both CI jobs pick it up with **no new step** — what `visual.yml` gains is the arithmetic, stated: **78 captures** (26 layout-bearing components × 3 widths) on top of the main matrix's 4 128, plus density 6 and scaffolds 2 = **4 214 screenshots across 4 235 tests**, all four numbers measured from `playwright test --list --reporter=json`, not estimated. Sharding stays ×4 *and the workflow says why*: the axis adds 90 tests to a 4 145-test suite (≈+2%, ~23 per shard) and the whole spec runs in 8.1 s generate / 5.7 s diff locally. The rejected alternative is recorded with its price — ×12 themes ×2 schemes ×2 dirs would be 26 × 3 × 12 × 2 × 2 = **3 744** captures, +90% on the entire suite, for information the single-viewport matrix already carries. The baseline cache key gained `src/utils/breakpoints.ts` beside the existing `tests/visual/**/*.ts` + `registry/**` hash, so a canon change reseeds baselines rather than silently diffing against the old ladder. The set is **discovered, never listed**: `discoverLayoutBearing()` filters `discoverComponents()` by the sibling manifest's `category === "layout" || kind === "pattern"` — 11 layout primitives/recipes + 15 patterns = 26 today. `tests/visual/responsive-matrix.test.ts` proves the *property*, not the count: it re-derives the set from every manifest on disk as independent ground truth, then hands the builder a **synthetic** component and shows it enters via the category clause, enters via the layer clause, and stays out under `category: "forms"` — zero suite edits in all three directions.)
- [x] Every pattern's structural collapse is pixel-proven at all three widths. (All 15 patterns are in by the layer clause, so 45 of the 78 captures are patterns at 390/768/1280 — and **every capture pre-asserts in the page before the shutter** (the 0.7-11 rule). `gatherLayoutFacts` reads what the browser computed — the `display` and used track count of every `[data-ui="grid"]`, the drawer's `position` and the shell's column count, the panes' visibility — and `checkLayoutFacts` compares it to the ladder the **markup** authored, resolved through 0.8-01's canon; never to the stylesheet, so a sheet that stopped implementing its own ladder fails instead of agreeing with itself. Three archetypes, all generic: grid (stats-dashboard's `data-cols="1" data-cols-md="2" data-cols-lg="4"` must compute 1/2/4 at the three widths; grid's own page carries nine grids including the `auto` intrinsic mode, whose count is deliberately *not* asserted because it derives from available space, and the `data-scroll` strip that must be `display: flex` below `sm` and a grid from `sm` up), the dashboard-shell drawer, the inbox panes. **The negative proof is run in the browser, not asserted in prose**: three cases inject an `!important` override that breaks one real rule — the grid's tier ladder, the drawer's `position`, the detail pane's `display` — assert the pre-assertion catches it with the right message, then re-mount the same page unbroken and assert silence. Nineteen `bun test` cases drive every branch of `checkLayoutFacts` from literal facts, including the inverse regression (a collapse firing *too early*), both sides of the `data-scroll` boundary, and the 640.5px width that fell through 1.x's `max-width`/`min-width` dead zone. 78/78 captured green on the first run; the three stats-dashboard captures were re-verified byte-identical against baselines taken *before* the a11y fix below, which is what proves `tabindex` is visually inert.)
- [x] Mobile axe sweep green with zero new exemptions. (`tests/a11y/mobile.pw.ts` scans the identical set at 390 px — **26 × {default, contrast} × {light, dark} = 104 scans**, 105 with the tripwire — through the same `AxeBuilder`/`partitionViolations`/`formatViolations` pipeline as the desktop gate, importing the *same* `discoverLayoutBearing`; `a11y-matrix.test.ts` asserts it is the identical function object, that the two gates' page sets are equal, and that the mobile set is a strict subset of the desktop scan's, so no page is ever scanned only on mobile. It immediately found what the task predicted it would: **one violation the 1280 px scan structurally cannot see** — `scrollable-region-focusable` (WCAG 2.1 A) on stats-dashboard's five-column sales table, in all four theme×scheme cells. `[data-ui="table"] { overflow-x: auto }` makes the root a scroll container at *every* width, but it only actually scrolls when narrow, so the desktop scan is silent by construction. **Fixed at the source, not exempted**: `tabindex="0"` on both of that pattern's table roots — the second one passes today only because its content happens to fit, which is content luck — pinned by a new case in `tests/patterns/stats-dashboard.test.ts` and re-verified pixel-neutral. The exemption list is byte-unchanged at three entries, and "zero new exemptions" is now **structural rather than observed**: a meta-test asserts an entry has exactly `{rule, component, justification}` with no viewport dimension, so a mobile-only waiver is unrepresentable, and none of the three waived components (`label`, `switch`, `slider`) is even in the mobile set. 105/105 green; the full a11y config is green end-to-end (660 passed, 0 failed, 18.5 min locally). The *general* contract — the `table` recipe creates the scroll container, so arguably every consumer deserves keyboard access, not just the page this gate caught — is filed as follow-up **0.8-16** rather than swept into a test-coverage task.)
...
> **Superseded as of 0.9-01 by `tests/visual/layout-budget.json`**, which is the same sweep
> with its rules written down and committed: **180 pages** (the 86 examples *and* the shell
> pages) → **79 zero-gutter, 42 seam-bearing pages, 183 seams, 7 bleeds, 3 overlaps**. The
> table above is the estimate that motivated the phase; the budget is the number a task is
> measured against. The three deltas — a centred measure column counts as a gutter, only
> stacked *block* demos can form a seam, and a box behind a live scrollbar is reachable and
> therefore not a bleed (so `carousel` is a gutter finding, not a bleed one) — are argued in
> 0.9-01's first acceptance note.
...
**Acceptance criteria**
- [x] Layout-lint runs over the built site and its committed budget reproduces the measured baseline (84 zero-gutter, 44 seam-bearing pages, 187 seams, 1 bleed, 1 overlap). (**Measured, and the measurement disagrees with the estimate in three places — the numbers below are what a rule you can read produces, not what an unrecorded script produced.** `tests/visual/layout-budget.json` covers **180 pages** — all 86 examples *and* the shell pages, which the estimate did not include — at 1280×900: **79 zero-gutter, 42 seam-bearing pages, 183 seams, 7 bleeds, 3 overlaps**, of which the example-page slice is 79 / 42 / 183 / 7 / 3. Where it differs and why: (1) **gutter** is measured on the content root's *outer* in-flow children, so a page whose content is a centred measure column has one — `document`, `form-page` and `wizard` do, which the estimate's "no padding/margin on body/main" reading missed, and `watermark`/`toast` paint nothing but fixed boxes so they have no gutter to measure at all; 86 − 5 = 79 rather than 84, and both `sidebar` and `dashboard-shell` are non-zero for the reason the estimate gives (24px of their own padding). (2) **seams** are counted only between *stacked block* demos — a row of inline badges is 0.9-08's problem, not a rhythm defect — which is the 183/187 and 42/44 delta. (3) **bleed** asks *reachability*, not clipping: a box behind a live scrollbar is one gesture away, so the docs shell's 122 long `<code>` lines inside `overflow-x: auto` are not findings (counting them would put a permanent floor under a number 0.9-11 must drive to zero), and by the same rule `carousel`'s eight slides sit in a scroller with a live scroll range and are *not* the bleed the estimate called them — the carousel's real defect is that its viewport is flush at x=0, which the gutter condition counts. What the rule does find is seven boxes at **x = −4** inside `dashboard-shell` (4) and `sidebar` (3): LTR `scrollLeft` cannot go below 0, so that content is genuinely unreachable. (4) **overlaps** land exactly on the estimate: `toast`, three pairs. The four conditions live in `src/utils/layout-lint.ts` — pure, `node:*`-free, 28 `bun test` cases over literal rectangles, each in both directions — and `tests/visual/layout-lint.pw.ts` supplies boxes from a real browser, with a synthetic clean page and a synthetic page carrying exactly one of each defect proving the *collector* hands over the right ones. The budget is regenerated from a fresh run and compared for equality on every run, the `check:docs` determinism shape.)
- [x] The budget can only decrease; a rise in any of the four counts fails CI. (Five counts, not four — `seamPages` rides along with the four. `compareBudget` returns regressions and slack separately: a rise is a failure that names the count, its before/after and the pages responsible; a fall passes and prints `zeroGutterPages: 79 → 0 (79 better than budget)`. Proven in `bun test` from synthetic budgets in all four directions — worse fails, better passes with slack, unchanged is silent, a *new* page that adds to a count is blamed as `path (new)` — plus the property that `pages` is never ratcheted, so adding a clean page is not a regression. The ratchet is not bypassable from the update path either: `bun run lint:layout:update` refuses to record a rise and points at it, so the committed file can only ever describe a better site than the last commit did. The gate rides inside `bun run test:visual` (its four tests are serial, so sharding keeps them in one shard) and `bun run lint:layout` runs it alone.)
- [x] No padding exists in the visual harness that the shipped artifact lacks; `bun run test:visual` is green on a clean tree and follow-up 0.8-14 is resolved (edit its row in place per house rule). (The harness now injects **no CSS at all**: `body { padding: 24px }` went, and so did `main.vr-root { display: flex; … gap: 16px }`, which was the same lie in a more damaging form — a 16px gap between every top-level demo is precisely what a suite hunting zero-gap seams must not add. The remaining declarations were redundant with `registry/base/reset.css`, so the block is gone entirely and a capture is of shipped bytes: `renderAgentSnippet` is `<body><main>` + the fragment, and `site/styles/docs.css` sets only `background` on `body`. `matrix.test.ts` pins it (one `<style>`, no `padding`/`gap` after it, no rule for the mount point, no inline style). **Re-baselined and green: 4 243 passed**, 0.8-14 resolved in place. The parity fix also had a consequence worth recording rather than papering over: `align-items: flex-start` had been shrink-wrapping every fragment, so scroll containers never actually scrolled and `test:a11y` went **red with 8 mobile failures** the moment the harness told the truth — `document`'s table root and `grid`'s `[data-scroll]` strip are unreachable by keyboard at 390px. Both fixed at the source with `tabindex="0"` (pixel-neutral; `registry-index.json` + docs site regenerated), the systemic half appended to follow-up **0.8-16**, which had predicted exactly this ("content luck, not a guarantee"). All gates green afterwards: `bun test` 3 124, `typecheck`, `check:docs` (302 files), `check:skill`, `check:registry-index`, `check:schema-refs`, `check:audit-browser`, `check:bindings`, `audit:registry`, `size`, `test:a11y` 672, `test:visual` 4 243.)
...
**Acceptance criteria**
- [x] One rule, written in FAQIR-SPEC.md §20, that produces vertical rhythm with no per-element opt-in — with the two rejected candidates and *why* recorded, because the next spacing question will ask again. (**Candidate 3 wins, and the ordering in the task header turns out to be backwards: candidates 1 and 2 are eliminated by the header's own test rather than tried.** "An agent can be told the rule once and produce spaced output *without wrapping anything*" is a test a wrapper cannot pass, and both a `flow` primitive and a `data-flow` prop are wrappers — the first also spends a sixth layout primitive days before the 1.0 freeze, the second grows one attribute across three manifests, which is the drift the manifest system exists to prevent. Both are recorded in §20 "Rejected candidates" *and* as data in `RHYTHM_REJECTED`, so the docs site and llms-full.txt print the reasons rather than the decision alone. §20 "Rhythm" states it in one sentence: **"Inside a flow root, consecutive block-level Faqir components and nested flow roots are separated by `--flow-space` of vertical space."** Three words in it are load-bearing and each is argued in the spec and pinned by a browser test. **"Flow root"** — the HTML sectioning elements plus `container` and `surface`, and *never* an element carrying another `data-ui`, because `stack`/`cluster`/`grid`/`switcher` already space their children with `gap` and a margin on top would silently double every stack in the registry. **"Block-level"** — CSS cannot select on computed display and a vertical margin on an `inline-flex` box shifts it inside its line box instead of stacking it, so the 26 inline-level components are named in the CSS; the list is *derived*, not hand-kept, from every component's root `display` with a UA-default fallback for the tag (which is what puts `link`, whose CSS declares no display at all, in it), and `tests/utils/rhythm.test.ts` recomputes it and fails if the CSS, the module and the registry disagree. **"Consecutive"** — this one was measured, not reasoned: the first draft spaced every non-first child, and the printed `invoice` scaffold came back with every heading floating 48px above its own section. The margin now goes on the second of two *participants*, so a non-participant between them suppresses it and an `h2` stays welded to the table it labels; `test:visual:print` is byte-identical again. Implementation is `registry/base/rhythm.css` — one rule, at (0,0,0) specificity (`:where()` around every list, a `:where()` argument inside every negation, asserted by stripping them and requiring nothing to remain), plus the `data-gap` ladder scoped to flow roots. `data-gap="0"` is the opt-out and `data-gap="8"` a re-tune, on the same `0 1 2 3 4 6 8 10 12 16` ladder the primitives take, so no new attribute name enters the protocol before 1.0. Ten Playwright cases in `tests/visual/rhythm.pw.ts` prove it in a browser on all three required shapes and two guards: nested scopes (48 inside *and* outside, never 96), the `surface` boundary (the first child sits exactly `padding + border` below the surface's top and the last exactly that above its bottom — the collapse that made this candidate look dangerous cannot happen, because the owl never puts a margin at a box edge), `data-gap="0"` → 0 and `data-gap="4"` → 16 with inheritance into a nested root, a row of badges staying a row on one baseline, and a `stack data-gap="4"` keeping exactly 16 rather than 64. **No component's own default margin moved** — the rule is keyed on the parent and lives entirely in base CSS — so no `breaking: true` manifest changelog entry was due; the change is recorded in §20 instead, which is where a base-layer change belongs.)
- [x] The rule is data in `src/utils/layout.ts`; README, `docs/layout.md`, skill, context and llms.txt all teach it from that one source, cross-checked by test. (`RHYTHM_RULE`, `FLOW_ROOTS`, `FLOW_ROOT_TAGS`, `INLINE_LEVEL_COMPONENTS`, `FLOW_SPACE_PROPERTY`, `FLOW_SPACE_DEFAULT_TOKEN`, `FLOW_GAP_ATTRIBUTE` and `RHYTHM_REJECTED` are the 0.8-12 one-module shape; `rhythmLine()` is the paragraph six surfaces print verbatim. It reaches `context.json` as `layout.flow` (rule, property, default token, roots, the 26 exclusions, opt-out, both rejections), llms.txt beside the primitive list so an agent that reads only the index still learns it, llms-full.txt with the rejections expanded, the markdown and cursorrules formats, `SKILL.md`, the docs site's Layout page as a new `#rhythm` section, README and `docs/layout.md`. A seventh copy rides in `LAYOUT_RULES` — "vertical rhythm is the default, not an opt-in… do not wrap a sequence in a `stack` merely to separate it" — because that is the rule an agent is most likely to violate by habit. `tests/utils/rhythm.test.ts` is the drift gate, built in the `tests/utils/breakpoints.test.ts` shape: §20's rule sentence, flow-root list, exclusion list *and count*, default token and opt-out are re-parsed out of the markdown and compared to the module; `registry/base/rhythm.css` is parsed back into selector/body pairs and its flow roots, both halves of the owl, its exclusions, its property, its ladder rungs and its zero specificity are compared to the same module; §20's three HTML examples pass `faqir audit` at zero findings, every severity, and the first is asserted to wrap nothing in a layout primitive — the claim the whole decision rests on. 27 cases; the file also asserts rhythm.css is installed by all seven surfaces that ship base CSS, since a base rule nobody copies is not a default.)
- [x] Layout-lint's seam count falls measurably from fragment-free changes alone; no registry reference fragment edited in this task. (**seams 183 → 19, seamPages 42 → 5**, from one new CSS file and its wiring — `git status` carries no `registry/{primitives,recipes,patterns}/*/*.html`. `overlaps` fell 3 → 1 as a side effect worth naming: `toast`'s three fixed-box pairs became one because the rhythm pushed the stacked regions apart, which 0.9-06 now inherits as a smaller problem. **Superseded by 0.9-03, and the correction is the more interesting fact:** it went back to 3 the moment each demo got its own captioned block, because that was never rhythm — §20 puts `margin-block-start` on the second of two *consecutive* participants, and `toast`'s regions are `position: fixed`, so what separated them was one of the three happening to be a first child and the other two happening to carry a flow margin that a box outside the flow has no business receiving. Any element between two demos ends that, uniformly, for all of them; see 0.9-03's third acceptance note. 0.9-06 inherits the problem it always had. `zeroGutterPages` is unchanged at 79 **and asserted to be** — the rule is vertical only, the gutter belongs to 0.9-03, and pinning it is what keeps the seam win attributable to §20 rather than to something else that moved. The delta is an assertion, not an anecdote: `tests/utils/rhythm.test.ts` names 0.9-01's committed baseline (183/42/79/180 pages) and fails if the seam counts stop being lower or the page count stops matching, so a later task cannot quietly re-cramp the site and still look green. The 19 that remain are the fragments' own fault and are 0.9-04's: `separator` (7), `key-value` (5), `description-list` (4), `radio` (2), `signature` (1) — every one a run of demos that are not consecutive `[data-ui]` siblings.)
...
**Acceptance criteria**
- [x] All 86 example pages carry a gutter and a measure from registry primitives; layout-lint's zero-gutter count reaches 0. (**`zeroGutterPages` 79 → 0**, measured over the same 180 pages at 1280×900 and re-recorded in `tests/visual/layout-budget.json`. 84 fragments are mounted in `<div data-ui="container" data-measure="wide" data-gutter="4">` — `wide` is the docs shell's own measure, so a component is demonstrated at the width the site reads at rather than stretched across the window, and both values are the primitive's declared vocabulary (the test reads them out of `container.manifest.json` and fails if either stops being declared, or becomes the `full`/`0` opt-out). The column **wraps** the landmark rather than being it, which is what `container`'s own a11y note asks for. The other two are the exemption and it is content-derived, never a list: `sidebar` and `dashboard-shell` declare their own `<main>`, so they are describing a whole document — an app shell that runs edge to edge and paints its own 24px inset — and capping one at 72rem would misrepresent the thing it demos. `hasOwnMain` decides it, the test asserts the set is exactly those two, and layout-lint already measured a gutter on both. A second number fell for free and is worth naming: **`seams` 19 → 0, `seamPages` 5 → 0** — `separator`, `key-value`, `description-list`, `radio` and `signature` welded their demos together because those demos are not `[data-ui]` siblings, and `figure` *is* a §20 participant, so wrapping each demo spaced them without a fragment being touched. That is 0.9-04's first layout-lint criterion, met here.)
- [x] Demo captions render from the fragments' existing comment labels; no fragment edited in this task. (**388 labels across 84 pages**, and `git status` carries no `registry/**/*.html`. `splitReferenceDemos` scans a fragment for comments at nesting **depth 0** — `sidebar` writes `<!-- ── Sidebar ── -->` inside its shell and that is an annotation, not a boundary — and each slice becomes a `<figure data-docs-demo>` with a `<figcaption>`. `figure`/`figcaption` rather than a section and a heading for two reasons that both bite: it is literally what the elements are for, and it keeps these pages out of the heading outline, so a caption cannot collide with the fragment's own headings under the `heading-order` document rule (which runs over every example page and is green). The caption is the first line of the *nearest* label, so `aspect-ratio`'s paragraph of orientation prose above its first demo becomes a note under the label rather than out-shouting it; lines with no letter or digit are dropped first, so `sidebar`'s box-drawing rules never become the visible text. Proven in both directions and derived rather than sampled: `badge` renders its three labels verbatim; every fragment whose comments carry no label at all (`context-menu`, `file-upload`, `menubar`, `tree-view`, `form-page`) is asserted to emit a `<figure>` and **no `<figcaption>`** rather than an empty one; and every caption on the site is matched, character for character, against the registry file it came from. The claim that made the audit split honest — "an example page IS its registry fragment" — is now proven as a *reconstruction* rather than a substring: each block's markup must appear verbatim, and the blocks rejoined must equal the fragment with comments stripped. That is the stronger form of the same statement. The `zz-probe` fixture grew a `<!-- Probe row -->` label and its test asserts the caption block and the column alongside the unchanged **+3 files**. **One count rose and it is the honest number:** `overlaps` 1 → 3 on `examples/recipes/toast.html`. 0.9-02 had knocked it 3 → 1 by pushing the stacked regions apart, but those regions are `position: fixed` and §20's margin lands on the *second of two consecutive participants* — so the separation was one region being a first child while the other two carried a flow margin an out-of-flow box should never have had. Any element between two demos ends that for all of them equally, which is why no wrapper, `display: contents`, participant-caption or per-block offset recovers it (uniform offsets move three boxes anchored to one corner by the same amount). The alternatives were worse than the truth: containing the fixed boxes to their demo block would blind the metric that 0.9-06 exists to drive to zero, and naming `[data-ui="toast"]` in `docs.css` would be the per-component escape this site claims not to have. So the budget records 3 — the number 0.9-01 measured for a defect nobody has fixed yet — and 0.9-06's acceptance already says it reaches 0.)
- [x] Follow-up 0.8-13 resolved (edit its row in place); the twelve theme frames carry no inline style and their baselines are re-captured in this commit. (Row edited in place and flipped to ✅. The frames' `<main style="padding: var(--space-4); display: grid; gap: var(--space-4);">` is now `<main data-ui="surface" data-variant="flat" data-size="md" data-gap="4">`: `surface` is the primitive that pads on both axes — `data-size="md"` *is* `padding: var(--space-4)` — which is the half `container` could not express and the reason the row existed, and the rhythm half needed no primitive at all because 0.9-02 made `surface` a flow root. **The re-baseline is a smaller claim than the row assumed, and saying so is the honest reading:** no screenshot baseline covers these twelve frames — `tests/visual/matrix.ts` builds its documents from registry fragments rather than from this generator, and `frames/**` is deliberately outside the layout-lint sweep because a frame is sized by its parent `<iframe>`. Their real gates were re-run and are green: `test:a11y` (**672 passed**, every frame axe-scanned), `docs-switcher.pw.ts` (**22 passed**), and `docs-themes.test.ts`, whose swatch assertion moved from an inline `background:` to the attribute and now also checks that the generated rule painting it is in the stylesheet. While here, the general case: **`docs-site.test.ts` fails on any `style=` attribute on any page the generator authors** (extending 0.8-03's `flex-wrap` assertion, as the task asked), which took the other three escapes with it — the token-page and gallery swatches became `<span data-docs-swatch="<token>">` painted by rules `buildSiteStylesheet` generates from the same token reference the pages are derived from, and the playground preview frame became `[data-docs-playground-frame]` in `docs.css`. Example pages are excluded from that assertion on purpose and by the existing split: their bodies are registry markup, and `sidebar.html` ships an inline `style` of its own — a registry defect, not a site one. The 0.7-13 split itself gained a tripwire in both halves: no example page is in the strict gate's set, **and** the per-component rules are asserted to still fire on that markup, so the split cannot quietly become cosmetic.)
...
**Acceptance criteria**
- [x] Layout-lint reports 0 seams and 0 zero-gutter pages across the library; the ratchet floor is set to 0 so it cannot regress. (**seams 0, seamPages 0, zeroGutterPages 0 over 180 pages at 1280×900** — `tests/visual/layout-budget.json`, unchanged by this task's 27 edited fragments, which is the more useful fact: 0.9-02 and 0.9-03 had already driven both to zero from the rhythm and the shell, so what 0.9-04 owed was proof they *stay* there. The floor is the committed budget itself and `compareBudget` fails any rise, so zero is a wall in the only direction that matters. Pinned in `bun test` rather than only in the playwright run: two new cases in `tests/utils/layout-lint.test.ts` — one seam on one page, and one zero-gutter page, each asserted to fail against a zero floor and to *name the page responsible* (a budget of 0 is not a special case in `compareBudget`, and now it cannot become one), plus a case that reads the committed file and asserts its three ratcheted counts are 0 over ≥180 pages. The two numbers that are not zero are recorded and owned elsewhere: `bleeds: 7` (`sidebar`/`dashboard-shell`, 0.9-11) and `overlaps: 3` (`toast`, 0.9-06, whose acceptance already says it reaches 0).)
- [x] Layout primitives demo with visible bounds; the mechanism is chosen once and applied to all five. (**The demo cell is `<div data-ui="surface" data-size="sm">`** — the registry's own primitive, whose default (no `data-variant`) is exactly a background, a border and a radius, which is the whole of what a cell has to be. Chosen over a docs-only bounds toggle for the reason 0.9-03 gave when it refused to name a component in `docs.css`: a toggle would make the page and the copy-for-agents payload two different things, and the payload is what an agent pastes. 77 cells across the five: **grid 37, stack 20, cluster 5, switcher 5** (its body-only `card`s — twice the markup for the same box; the two richer cards with headers and titles stay cards, because there the card is the demo), and **container 7**, where the invisible thing was not the cells but the column edge, so the surface bounds each measure column and you can finally see where 48rem ends. `grid`'s nine demos were the worst case in the task description — floating words with no cell, border or visible gap — and are now nine readable grids. Asserted rather than eyeballed: `reference-contract.test.ts` requires `data-ui="surface"` in all five files **and** that no bare `<div>text</div>` / `<span>text</span>` child survives in any of them, so a later edit cannot quietly reintroduce the floating-word demo.)
- [x] Follow-up 0.7-17 resolved (edit its row in place) with the reference-page audit contract written down and the registry at zero under it. (Row edited in place and flipped to ✅. **367 → 0** over all 87 fragments, and the row's own framing turned out to be the wrong one: it assumed a triage between "complete the reference", "declare the slot" and "scope the rule out", but **229 of the 367 were the audit being wrong** — 190 from reading composition as nesting (`data-part` attributed to the *nearest* `data-ui` rather than the nearest one that **declares** the slot, which reported `auth-form`'s own parts as the card's and then reported `auth-form` as missing them) and 39 from five rules that flagged correct markup, including a `close-label` that demanded an `aria-label` which would have *overridden* the visible "Cancel" and broken voice control. Only 99 were the defects the row predicted, and they were fixed rather than excused: ~60 styled-but-undeclared parts declared, `table`'s ad-hoc boolean variant groups normalised to the one group its CSS selects, seven `card`s given the `body` their contract requires, two `badge`s that are `chip`s, and nine `required: true` flags relaxed with a written reason each. **The contract, in three sentences, all content-derived and none of them a path list:** a *fragment* (no doctype/`<html>`/`<body>`) cannot carry a runtime, so `controller-loaded` and `focus-trap` do not apply to it — and apply in full the moment the same markup is a page, which is why the copy-for-agents payloads are now asserted clean under the **whole** rule set and the exemption is a scope rather than a hole; a `data-part` belongs to the nearest enclosing component that *declares* it, while `required-slot` asks only whether the slot is filled anywhere in the subtree; and `required` describes the **default** state, so a root carrying a declared non-default `data-state` is demonstrating that state, not failing to be complete. It is written down in `FAQIR-SPEC.md` §9 (a new "What a rule is asked about" section) rather than only in this plan, because `faqir audit` applies it in user projects too — which was the half of 0.7-17 that reached users, and a fresh `init` + `add crud-table` now reports nothing. Enforced in three places: `audit:registry` **gate 7** (the full rule set over all 87 fragments, previously only the document rules — which is why 367 findings sat here unseen), `tests/audit/reference-contract.test.ts` in `bun test` (zero findings, plus the fragment/page scoping proven in both directions and the two exempt rule ids pinned by name), and `tests/generator/docs-agents.test.ts` for the payloads. The per-file recomposition guarantee is asserted rather than asserted-of: `tests/fixtures/registry-component-roots.json` captures the component multiset of all 87 fragments **from `git HEAD` before this task**, and every file must still parse to it — except six named, justified entries (the five layout primitives' cells and `crud-table`'s badge→chip), with a second case asserting the changed set is *exactly* those six, so a stale exemption cannot sit there excusing something. One collision was found and deliberately not fixed: `empty-state` is two components sharing a name, filed as **0.9-13** and made harmless here by aligning both readings.)
...
**Acceptance criteria**
- [x] Trigger parts have one documented contract, applied registry-wide and enforced by a rule rather than by review. (**Two satisfying forms, never zero**: a trigger either carries a `data-ui` — delegating its look to a primitive, which is what `alert-dialog` already did and why its page alone looked right — or its own recipe declares a rule for `[data-part="trigger"]`, which is what the nine recipes whose trigger is *not* a button visually already did (`tabs`, `accordion`, `select-custom`, `date-picker`, `dropdown`, `popover`, `menubar`, `sidebar`, `collapsible`). Encoded as `trigger-contract` in `src/audit/rules.ts` — severity `error`, auto-fixable toward the delegating form — which found exactly 16 violations, all four triggers of `dialog`, `drawer`, `sheet` and `tooltip`, and zero false positives: `inbox`'s and `dashboard-shell`'s triggers belong to the `tabs`/`dropdown` they compose and are answered by *those* sheets, because attribution is 0.9-04's composition-aware `ParsedComponent.parts`. It is the first rule that reads a component's markup AND its stylesheet, so `HtmlAuditInput` grew an optional `styles` map — supplied by `runAudit` from `ui/**`, by the registry gate from `registry/**`, and by `createAuditor(manifests, styles)` in the browser; a component whose sheet the caller cannot produce is *skipped*, never guessed at, the same stance `undeclared-attribute` takes toward a missing manifest, and both halves of that are asserted so "silent" can never be mistaken for "never ran". It ships as a fourth scope, `markup+css` — the playground counts only what it can run, and now says so. 11 unit cases in `tests/audit/trigger-contract.test.ts`, 5 parity cases through the committed browser bundle, and the registry-wide gate is asserted over **all 87 fragments** rather than the four that were broken, so a recipe added tomorrow inherits it.)
- [x] All eight overlay recipes show their panel on the docs site without shipping an open state to users. (**The eight are eight — but three of the task's names were wrong.** Derived rather than listed: `rendersOnlyTriggers()` walks each fragment and asks whether anything but a trigger survives outside a `hidden` ancestor, and the answer is `alert-dialog`, `command-palette`, `dialog`, `drawer`, `dropdown`, `popover`, `sheet`, `tooltip`. `tree-view` and `file-upload` — two of the names in this task — render their whole structure statically and were never in trouble; `dropdown`, `popover` and `tooltip` were, and `command-palette` rendered a **completely empty page**. `context-menu` and `menubar` are added editorially (their fragments show something, but the menu — the substance — never appears), for 10 previewed pages against a gate of 8. The mechanism is `OVERLAY_PREVIEW_SURFACES` → two generated CSS rules per surface in `styles/faqir.css`, scoped to `[data-docs-overlay="<name>"]` on the example page's `<html>` — so a `dropdown` inside `dashboard-shell` stays shut on the shell's own page. The panel is put back **in flow** (`position: static`), not shown where it really opens: four fixed panels on one page would cover the triggers and, since 0.9-01 counts overlapping fixed boxes, turn every one of these pages into a finding — `lint:layout` confirms the budget did not move by a single count. The table has to record a `display` because `reset.css` ships `[hidden] { display: none !important }`, the one `!important` in the base layer, and a blanket `block` would relayout three flex columns. `tests/a11y/overlay-preview.pw.ts` asserts all of it in a real browser: 10 pages × a measured box, the resolved display, zero horizontal overflow, the page's own `<main>` placement unchanged, and axe-clean in both schemes — 31 cases, and the whole a11y suite is green at 703. **Rendering the panels found three real defects that had been invisible precisely because nothing rendered them**: `command-palette`'s group labels at 4.24:1 light / 3.47:1 dark (`--color-fg-subtle` → `--color-fg-muted`, the token every other secondary label uses), `alert-dialog`'s destructive title at 4.22:1 in dark (the tone is now the panel border and the confirm button — exactly how `dialog`'s danger variant already did it), and `dialog`/`drawer`/`sheet`'s icon-close rule flattening the footer's `<button data-ui="button" data-part="close">Cancel</button>` into a 32px borderless square — fixed with `:not([data-ui])`, which is the trigger contract read backwards: a part that delegates to a primitive must not then be restyled by its recipe. Visual baselines regenerated; `test:visual` green at 4 253, print at 4.)
- [x] No Copy-for-agents payload changes as a side effect. (Asserted by building the site twice — `buildDocsSite()` and `buildDocsSite({ overlayPreview: false })` — and comparing bytes: all 86 snippet payloads are identical and none contains the attribute, every example page differs by exactly the one `data-docs-overlay` attribute on `<html>` and nothing else, and the site stylesheet is the *same bytes with one section appended*, so the preview cannot have perturbed a token, a base rule or a component. 0.9-04's "an example page IS its registry fragment" still holds unchanged, which is the reason this is CSS rather than markup in the first place.)
...
**Acceptance criteria**
- [x] Card rows align part-for-part across siblings with unequal content, via a declared attribute rather than per-instance padding. (`grid` 2.1.0 declares the boolean `data-align-rows` prop and makes every direct card span one fixed four-track contract: `header`, `divider`, `body`, `footer`. `card` 1.1.0 declares the divider slot and the ordered composition; `pricing` 1.2.0 now builds each tier set from the real grid primitive and supplies all four direct rows. The featured card's extra badge expands the shared header track, while auto space inside that track also aligns the nested prices. `tests/visual/card-row-alignment.pw.ts` measures every direct row plus the price on each visual line at 390, 640, 768, 1024 and 1280px. The 51 pricing theme/direction/responsive visual cells were re-baselined locally, and the reviewed default diff shows prices, dividers, feature lists and CTAs sharing their intended lines.)
- [x] A correct non-subgrid fallback exists and is tested. (`@supports not (grid-template-rows: subgrid)` makes every participating card span `1 / -1`; the card keeps its intrinsic flex layout and complete source order, so an unsupported engine trades density for a truthful one-column stack instead of displaying falsely aligned peers or relying on guessed heights. The browser test disables the subgrid feature query, then proves all three complete cards are full-width, vertically ordered, intrinsically laid out and contain the same four ordered rows. The failure mode and the `data-scroll` incompatibility are explicit in grid's manifest, CSS, reference note, README, spec and layout guide.)
- [x] `docs/layout.md` states the equal-height vs alignment distinction and names the mechanism for each. (The new “Equal height is not alignment” section names `switcher`/ordinary stretching for equal outer boxes and `grid[data-align-rows]` + subgrid for internal card landmarks, fixes the contract at four rows, explains wrapped-line scope, and documents the full-width fallback. The same rule lives in `src/utils/layout.ts` so regenerated context and `.claude/skills/faqir-creator/` surfaces cannot drift; FAQIR-SPEC §15 and README carry the public summary.)
...
**Acceptance criteria**
- [x] A documented elevation ramp with a minimum adjacent delta, enforced across every theme and scheme. (`--color-bg` → `--color-surface-1` → `--color-surface-2` is now a four-pair contract: both adjacent fills plus each surface against its matching border must keep OKLab ΔE ≥ 0.03. `surface-elevation` is a first-class error rule in the CLI and browser audit inventories; it resolves the same theme/base token cascade as contrast, reports missing/translucent/unparseable ramp colors instead of skipping the guarantee, and is also applied before generated themes are written. The shipped-theme test discovers all 12 stylesheets and proves all four pairs in both schemes, so a thirteenth theme enters automatically. README, FAQIR-SPEC §15, theme templates, theme generation, all theme manifests and the registry index carry the same token surface.)
- [x] Follow-up 0.7-19 resolved (edit its row in place); no `<sem>`/`<sem>-subtle` pair below AA in any theme. (`CONTRAST_PAIRS` now includes primary/destructive/success/warning/info text on their subtle fills, and a text-bearing translucent pair is a finding rather than the old decorative escape. All 12 themes use opaque, tuned pairs and pass the static AA gate in both schemes. Visible text that had been relying on the deliberately decorative `color-fg-subtle` weight moved to `color-fg-muted`. The badge row is restored to every generated theme frame, and `A11Y_THEMES` now discovers all themes: the complete **2 973-case** Chromium axe run is green with the exemption list unchanged.)
- [x] Dark-mode `card` variants are visually separable — asserted by the ramp check, not by eye. (`card` 1.2.0 maps default/outlined to surface 1 and filled to surface 2, including the matching checked borders; `surface` 2.1.0 maps flat/raised/overlay across bg/surface 1/surface 2. The regression test ties card CSS directly to the checked tokens. The full ignored local visual set was regenerated unscoped (**4 214 screenshots / 4 264 tests**), and a subsequent 201-case comparison covering badge, card, surface and dashboard-shell across their theme/scheme/direction axes passed. Canonical pixel baselines remain the pinned-Linux CI cache by repository policy.)
...
**Acceptance criteria**
- [x] No shipped manifest names a file that is not there. (All **twelve** themes now have a `<name>.preview.html`: the five missing ones — `brutalist`, `default`, `document`, `midnight`, `paper` — are rendered by `renderThemePreview` in `src/theme-preview.ts` and written by `bun run gen:theme-previews`, rather than copied a sixth time from a sibling. `preview` is REQUIRED by the frozen schema, so the field could not be dropped — which makes the FILE the thing that must exist, and both writers now guarantee it: `gen-theme-manifests.mjs` refuses to write a manifest whose preview is missing (seeded by moving `soft.preview.html` aside — "✗ No preview harness for theme(s): soft"), and `faqir theme generate`, which declared a preview it never wrote, now writes one beside every theme AND every `--document` companion. That one is inlined, not linked: `themes/` there is a drop folder with no registry beside it, so a linking harness would resolve to nothing — 141 KB, self-contained, opens anywhere. `tests/themes/theme-previews.test.ts` (8 tests): every declared preview resolves for all twelve; every local `href`/`src` in every preview — generated and hand-authored alike — resolves on disk, and each links its own theme; generated ∪ bespoke covers the theme set with no overlap and no gap, so a new theme must choose; each generated file equals a fresh render; no bespoke file carries the generated header; a light-only theme renders one panel and a dual-scheme theme a split; and linking and inlining read the same stylesheet list in the same order. Also fixed while in there: `gen:theme-manifests` was silently STRIPPING the `$schema` ref that `add-schema-refs.mjs` had added — it now writes it itself, and the twelve manifests regenerate byte-identical.)
- [x] `data-ui="prose"` is discoverable from the skill and from `llms.txt`. (Four surfaces, one derivation. `src/base-layer.ts` reads `registry/base/*.css`, takes each file's `@ui:base <name> — <blurb>` header, and reports the `data-ui` values it selects on **that no registry component claims** — today exactly `prose`, with `rhythm.css`'s dozen component selectors correctly yielding none. Nothing is listed: a fixture base stylesheet defining `figure` beside a `button` rule produces `["figure"]` with no generator edit, and a base file with no header is refused rather than guessed at. The default route was taken — documented as styling, not given a manifest — because it has no parts, no variants and no states. It now appears in the shipped `SKILL.md` (`## Base Layer`, all four base stylesheets named), in a project's own `.faqir/SKILL.md` under **that project's** output dir (`ui/base/prose.css`, not the registry path), in `llms.txt` (`## Base layer`) and `llms-full.txt` (`#base-layer`, which the index links), and on `components/index.html` — the page that lists every component, which is where a reader looked for it and found nothing, and which is itself built out of `prose`. `tests/base/prose.test.ts` holds all four; each was seeded by deleting its `lines.push` and observed to fail alone.)
- [x] `audit:registry` clean, including the base layer. (Six gates, zero findings, including the `styles/faqir.css` rebuild that `prose.css` moved. `prose.css` itself: `max-width: 65ch` → `max-inline-size: var(--measure-prose)` — Strict Rule 2 violated in the base layer, against a token whose value is exactly `65ch` — plus `width` → `inline-size`, `border-top`/`border-bottom` → `border-block-start`/`border-block-end`, and `margin-top`/`margin-bottom: 0` → `margin-block-*: 0`. Two sweeps, scoped to the file rather than folded into `audit:registry`, because the rest of the registry writes block-axis physical properties by house convention and remediating 100 stylesheets is not this task: no length literal may equal a declared token's value, and no physical property may have a 1:1 logical equivalent. Both exemptions are by construction, not by exception — border WIDTHS are skipped because this framework has no border-width ladder and matching `1px` to `--space-px` would swap a spacing token into a border, and `overflow-x` is skipped because `overflow-inline` is an axis keyword rather than a rename and six other registry rules write `overflow-x`. Reverting the one line fails both sweeps at once. Verified: 3,534 bun tests, typecheck, `check:docs` (331 files), `check:skill` (7 files), `check:registry-index`, `check:audit-browser`, `check:schema-refs` (98 manifests), `check:bindings`, `size`, the layout ratchet unchanged at both viewports, and all 251 docs-site axe scans green in both schemes.)
...
**Acceptance criteria**
- [x] A hallucinated `data-ui` value is caught, with a suggestion. (`unknown-component` in `src/audit/rules.ts` — a `RuleInfo` + `buildUnknownComponentResults`, because a rule about a name with no manifest cannot be an `AuditRule`, whose `check` takes one. It is decided from a new optional `HtmlAuditInput.knownUiValues`, and suggestions come from the registry's names ∪ the caller's manifests, sorted, so `dailog` always answers `dialog`. **The plan's own example does not get a suggestion and that is correct**: nothing in the registry is within `suggestClosest`'s three edits of `datatable` (`table` is four), so the finding names `faqir list` instead — the budget is the one the CLI dispatcher and `icon-name` already use, and widening it for this rule alone would trade a real hint for a plausible wrong one. No `fix`: every other suggestion here repairs markup a manifest proves wrong, whereas the closest *name* is a guess, and `faqir repair` applies fixes automatically.)
- [x] Not-installed components stay silent; mixed-framework pages stay usable. (Silence is the default the rule is built around: `knownUiValues` is optional exactly as `styles` is, so a caller who cannot produce the registry's names gets the pre-1.0R-11 behaviour rather than a guess derived from the manifests it holds — asserted in **both** directions, since "the rule is silent" and "the rule never ran" look identical from outside. Known is four sources, not one: component directories, **aliases** (`alert` is `callout` — the reason the set is read from the manifests and not from `registry-index.json`, which has no `aliases` field), base-layer values (`prose`, task 1.0R-10) and — found by pointing the rule at Faqir's own registry — the **companion values a component's own stylesheet defines**: `button-group`, `input-group`, `radio-group`, `radio-label`, `checkbox-label`, `switch-label`, `heading`. Seven `data-ui` values with no manifest anywhere, four of them used by patterns; `add button` + `faqir audit` reported the framework's own reference file until they were derived (from the **stylesheets**, never from the markup under audit — pinned on a synthetic registry, since in the real one every companion appears in both). Documenting them is follow-up **1.0R-13**. Severity is `warning`, so a page that deliberately mixes in another system's `data-ui` still exits zero; `faqir audit --skip-rules <ids>` (new, generic over the inventory, comma- or space-separated) silences it entirely, and the hatch is named in the message, in the rule's `exempt` list under `faqir audit --rules`, in `faqir help`/`SKILL.md` and on the playground page.)
- [x] `bun run check:audit-browser` green; playground legend updated automatically. (Bundle rebuilt and gated — 49.91 KB raw / 16.91 KB gzip, `check:audit-browser` clean. `createAuditor` takes the names as an explicit third argument and exposes them, because deriving "the registry" from a page's manifest payload is right only for a payload that happens to be complete; parity is asserted with the list and without it, and the bundle's rule inventory gained a fifth scope, `markup+registry`. The legend is now `getHtmlRuleInventory()` — the engine's own list of the rules a markup-only caller can run — so the row arrived with no site edit and no hand-written entry; `site/lib/playground.js` gained one argument, the names the generator ships beside the manifests (`window.__FAQIR_UI_VALUES__`, from the same `knownUiValues` the CLI calls), and a happy-dom test types a hallucinated name into the real page and reads the warning back. `audit:registry` and the reference-contract sweep arm the rule too — that gate promises "every rule a fragment can satisfy" and cannot quietly leave one unarmed. **3,564 bun tests**, typecheck, `check:docs` (331 files), `check:skill`, `check:registry-index`, `check:schema-refs`, `check:bindings`, `size`, `audit:registry` — all green. Six mutations seeded and observed to fail individually: dropped companions, dropped aliases, companions derived from markup, the not-installed skip removed, the browser dropping the names, and the rule removed from the legend inventory.)
...
**Acceptance criteria**
- [x] Spec + schema published at stable site URLs with version in the path. (`SPEC-1.0.md` is the normative document; the site serves it three ways under one versioned prefix built from `PROTOCOL_VERSION` rather than written out — `spec/1.0/` rendered, `spec/1.0/spec.md` byte-identical to the repository's file, and `spec/1.0/manifest.schema.json` byte-identical to the `$id` alias at the root. The alias stays: `/manifest.schema.json` always resolves to the newest 1.x, `/spec/1.0/manifest.schema.json` will still serve *this* schema after a 1.1 exists. All three are in `docs-agents.test.ts`'s hand-written literal path contract, carry generated `_headers` content types, and the rendered page is a normal site page under the normal audit + axe gate.)
- [x] Amendment process written (what requires a major version). (§8 of the spec, as a twelve-row decision table — five additive, seven major — plus the two cases the table deliberately does not cover, a severity raise and a rule bug fix, and the four-step proposal procedure. The table is not prose: `AMENDMENT_RULES` in `src/protocol.ts` carries the same rows, the spec's copy is parsed back out by marker and compared row for row in both directions, and the schema's `amendment_policy` points at the `#amendments` anchor the page actually emits.)
- [x] Zero validation or spec-example failures. (All 98 registry manifests validate against schema 1.0, which now declares `schema_version: "1.0"`, `stability: "frozen"` and its own 0.x changelog; the schema still meta-validates against draft-07. Every fenced block in the spec is verified by the checker that fits it — seven `html` examples audit to zero findings at every severity against the shipped manifests, the `css` block is asserted to obey the attribute-selector rule it demonstrates, the `json` block validates against the schema's own `variant`/`prop` definitions, and an unrecognised fence language fails rather than being exempted. Verification: **3,377 Bun tests**, root+workspace typecheck, all seven `check:*` gates, `audit:registry` clean, **2,985 axe cases**, the full visual suite, and layout-lint at 186 pages / 0 findings.)

**Session note (2026-08-17).** Three decisions worth recording:

1. **A new document, not a section of `FAQIR-SPEC.md`.** That file is 2,400 lines of design record — how the system was arrived at, day-by-day implementation phases included — and a frozen contract cannot live inside a document that is free to keep changing. `SPEC-1.0.md` says only what an implementation must do; `FAQIR-SPEC.md` §2 now opens with a pointer saying which of the two is normative.
2. **The spec is checked against code, not just written.** `src/protocol.ts` carries the five attributes, the three token modifiers, the amendment taxonomy and the schema changelog as data; the spec places an HTML-comment marker above each normative table and the suite parses them back out and compares. The alternative — prose that agrees with the engine on the day it is written — is exactly what a freeze cannot survive.
3. **`schema_version` is `1.0`, not `1.0.0`.** The file has claimed `1.0.0` since its first commit, before there was a freeze to back the claim; the changelog row records that rather than tidying it away. The patch digit said nothing, because under "additive only" a schema patch is not a thing that can exist. One consequence, absorbed here: the generated skill's header states the schema version, so `tests/generator/skill.test.ts` stopped asserting a three-part shape and now asserts the constant.
...
**Acceptance criteria**
- [x] `Faqir` fully typed including plugin extension points. (`packages/core/faqir-core.d.ts`, 730 lines, UMD-shaped — `export = Faqir` + `export as namespace Faqir`, so `import Faqir from "@faqir-ui/core"` and the `<script src>` global are ONE declaration rather than two that can disagree; wired as the package's `types` at the top level and in every `exports` condition, and added to `files`. Covers all 19 members of the global, the 8-key devtools handle, `Inspection`/`ProtocolState`/`InspectedDirective`, the `SourceController` that `l-source` injects as `$<name>`, the 11 page-visible magics and 29 controller APIs. The three extension points are augmentable interfaces — `PluginMagics`, `ControllerApis`, `Stores` — opened with `declare module "@faqir-ui/core" { interface X … }`; the form has NO `namespace Faqir` wrapper because `export =` makes the namespace's members the module's exports, which was determined by compiling both forms rather than remembered, and the working one is exercised in `tests/fixtures/types/augmentation.ts`. Two deliberate tightenings: `Scope` indexes to `unknown` rather than `any`, and `$ui` is `ControllerApi | null` (i.e. `destroy` only) since the engine returns whatever the controller returned — naming the component with `inspect<Faqir.ControllerApis["dialog"]>()` is what unlocks the rest.)
- [x] Runtime/declaration drift test in CI. (`tests/core/faqir-core-types.test.ts`, 46 cases. It parses the `.d.ts` with the TypeScript AST — not a regex, because the interfaces nest, extend one another, and carry doc comments full of the identifiers a regex would match — then holds every declared name against the thing that DEFINES it, in both directions: `Object.keys(Faqir)`, `Object.keys(Faqir.devtools)`, the live key sets of `inspect()`/`scopes()`/`components()` on a mounted fixture, the engine's own `@ui:magic` block through the existing `parseEngineVocabulary`, the `ctrl` literal through `parseSourceController`, and each recipe's `@ui:provides` header — 29 recipes, one case each, so a controller that gains a method fails the type it promised. The second gate is a compile: `tests/fixtures/types/` (3 files, 24 `@ts-expect-error` negatives over 21 numbered surface points) is compiled with `skipLibCheck: false` and must produce ZERO diagnostics, which asserts both directions at once — an unused `@ts-expect-error` is itself TS2578, so a declaration loose enough to wave misuse through fails exactly like one too tight to accept correct code. It runs in CI twice: inside `bun run test`, and as a new leg of `bun run typecheck`. The root tsconfig excludes the fixture directory, since its whole point is code that must not compile.)
- [x] Security doc reviewed against §A6's list — every stated risk covered. (`docs/security.md`, eight sections. Every claim was MEASURED in real Chromium against the shipped engine under three served policies, not asserted: under `script-src 'self'` the engine loads, bootstraps, clears `l-cloak` and mounts controllers, and then `<span l-text="count">` renders `""` instead of `1` — the failure is SILENT, which is what the doc leads with rather than "add 'unsafe-eval' or see errors"; adding `'unsafe-eval'` takes the same page to `1` → `2` on click; under `style-src 'self'` the cloak `<style>` sits in the DOM with `sheet.cssRules` empty while `l-show` and `:style` keep working, because CSSOM writes are outside CSP — so `'unsafe-inline'` buys `l-cloak` and nothing else. §A6's four items plus the two the task added (CSP-restricted guidance, generated-trusted vs user-supplied threat model) are each pinned by a case in `tests/generator/security-docs.test.ts`, 24 cases, which derives what it can instead of trusting prose: the `l-*` names the doc cites are checked against the engine's `@ui:directive` vocabulary, the "one injected stylesheet" framing against `createElement('style')` appearing exactly once, and the "which patterns need the evaluator" claim against the registry — its detector counts only VALUED attributes, which is exactly what separates `inbox`/`wizard` from `form-page`'s bare `<form l-data l-validate>`, a distinction the first draft got wrong and the derivation caught. Reachability is asserted on both entry points: the README (TOC, its own `## Security`, and a pointer from the engine section) and all four generated agent surfaces, via a new `security` block in `src/generator/context.ts` (context.json, context.md, the llms.txt index and llms-full.txt) — and the policy the doc recommends is compared directive-by-directive with the one the context emits, so the two cannot drift apart. Two findings the verification turned up are follow-ups rather than fixes here, both outside this task's Touches: 1.0R-14 (the site's own generated CSP has no `'unsafe-eval'`, so every live `l-*` demo it serves is dead) and 1.0R-15 (a nested `-->` in `site/content/engine.html` swallows the entire reactive-engine guide). Verification: **3,587 Bun tests** / 0 fail (+70), 49/0 in the dev-engine pass, root+workspace typecheck including the new fixture leg, and all eight `size`/`audit:registry`/`check:*` gates. No new dependencies — `typescript` was already a devDependency.)
...
**Acceptance criteria**
- [x] A real old project upgrades successfully with documented, bounded manual steps. (`tests/fixtures/v024-project/` is not a reconstruction: its fourteen components are the bytes `git archive v0.2.4` produces — old manifests, old stylesheets, old controllers — under a `loom.config.json`, a `.loom/` cache and a page written in the vocabulary those manifests declared. `tests/migration/upgrade-v024.test.ts` copies it, runs the seven steps `docs/migration-1.0.md` publishes **in the order `MIGRATION_STEPS` records** — the last case holds the executed order against that constant — and audits the result: **111 findings → one warning**, and that warning is the `landmark` rule firing on a date-picker popup inside `<main>`, which the registry's own reference markup would trip the same way (follow-up 1.0R-17). Zero conflicts, `faqir doctor` green, and the merged `table.js` and `dialog.js` are imported *from the migrated project* and driven — sort, open, close — so "controllers function" is the project's own files running, not the registry's. Three CLI defects had to be fixed for the path to exist at all, each pinned by a case: (1) the first thing a v0.2.4 project met was `No faqir.config.json found. Run 'faqir init' first.` — advice that writes a second config with an empty `installed` list beside a `ui/` full of components; `missingConfigMessage()` now reads the legacy config and names the rename instead, at all 17 call sites from one function. (2) `faqir init --force` reset `installed` to empty, which made "refresh the engine" impossible without hand-editing the config; it now keeps the inventory, remote registries and bundle settings and rewrites only what the registry owns. (3) The pristine backfill — the step that makes the merge possible — snapshotted **today's registry at today's version**, so the very next `upgrade` compared the registry against itself and reported all 53 components up to date; it now snapshots the project's own copy at the version that copy's manifest records, which is the only honest baseline available and is flagged `backfilled` with a warning that says so.)
- [x] No undocumented breaking change exists (test-enforced via changelog cross-check). (Two gates, because a changelog cannot be checked against itself. **Completeness**: every `breaking: true` entry in the registry — nine of them — has a marked section in `docs/migration-1.0.md`, and the doc invents none; the marker is an HTML comment (`<!-- @faqir:breaking field-group@2.0.0 -->`) so it is invisible in every renderer and unambiguous to parse. **The harder half**: `tests/fixtures/v024/surface.json` pins what the v0.2.4 release *honoured* — every attribute value each component's manifest declared or its stylesheet and controller selected on — extracted from the release tag by `scripts/gen-v024-surface.mjs` through `extractSurface`, the same function the test runs against today's registry, because a diff run by two readers proves only that the two readers agree. Five components lost vocabulary since, and the gate demands each loss be **named in that component's own `changes` notes**. Three were not, and this task wrote them: **`date-picker`** shipped the calendar extraction (0.4-10) as a *minor* bump with **no changelog at all**, losing seven parts — it is 2.0.0 with a breaking entry naming each; **`patterns/empty-state`** replaced `data-part="action"`/`"secondary-action"` with `actions` across 0.9-04 and 1.0R-07 while **staying at 1.0.0**, and had additionally been trimmed to `data-size="sm"` while its own stylesheet still styles `md` and `lg` — so `faqir audit` rejected markup the sheet renders; it is 2.0.0, the sizes are declared again, and the leftover `[data-size="lg"] [data-part="action"]` rule now names the slot that exists; **`table`**'s 1.x manifest declared `data-variant="true"|"false"`, values no selector ever matched, which its 2.0.0 note now records as the correction it was. The gate also refuses to pass vacuously: one case asserts the loss inventory is exactly those five.)
- [x] Doc published on the site. (`migration/index.html` — the rename table and the seven-step procedure rendered from `src/migration.ts` (the constants the end-to-end test executes), and one section per breaking change carrying **the document's own prose**, parsed back out by marker and rendered through a markdown subset, so the page cannot say something the guide does not. `migration/migration-1.0.md` is served beside it, verbatim, with a generated `text/markdown` content type — the same two forms the spec is published in, for the same reason. In the sidebar nav, in the sitemap, audit-clean, axe-clean in both schemes, and recorded in the layout ratchet at 0 bleeds / 0 overlaps at both viewports with every other total unchanged. Deliberately **not** in the top nav: 1.0R-09 recorded that the bar is `overflow: hidden` with nine links at 1280 and a tenth would clip one silently. Also reachable from the README, which until now documented neither `faqir upgrade` nor `faqir diff`.)

**Session note (2026-08-22).** Three decisions worth recording:

1. **`faqir upgrade` stopped trusting version numbers.** Comparing the v0.2.4 registry against today's, **24 of 53 components ship changed bytes under an unchanged version** — `dialog`, `toast`, `accordion`, `drawer` and `tooltip` had their controllers, stylesheets *and* markup rewritten while staying at 1.0.0. The old fast path (`compareVersions(new, base) === 0` → up to date) skipped every one of them permanently. `upgradeComponent` now always merges and lets the outcome decide: a component whose every file comes back `unchanged` is up to date, whatever the two numbers say. Bumping 24 versions instead was the alternative and is worse — it would write 24 changelog notes for changes that happened across a dozen past tasks, and it would leave the mechanism able to lose the next one.
2. **The migration doc is authored; the list it owes is derived.** The edit a break needs is a human sentence (`data-state="error"` → `data-state="invalid"`, wrap the grid in a `calendar` root), so the sections are written. What is generated is the *obligation*: which breaks exist, and which vocabulary disappeared without one. That split is why the site page renders the document's own prose rather than a second telling of it.
3. **What the migrated project could not reach, and why it is a follow-up rather than a fudge.** `faqir audit` on the finished project exits non-zero, and not because of the migration: `registry/primitives/switch/switch.css` carries an `rgba()` box-shadow that the `no-hardcoded-values` rule reports as an error, and `switch` arrives as a dependency. `audit:registry` never runs that rule over its own CSS, so the framework is not held to a rule it enforces on projects — filed as **1.0R-16**, with the end-to-end test asserting the residual findings are *exactly* those two so it fails informatively when they are fixed. Fixing it here would mean either changing three components' shadows (a visual decision with CI-pinned baselines) or adding a token; both belong to their own task. The fixture also drops `patterns/empty-state`, because `faqir add empty-state` in a project that installed the pattern resolves the *primitive* — the 0.9-13 name collision, filed as **1.0R-18**.
...
| 0.7-17 | **Registry reference pages are not clean under the per-component audit rules — found by 0.7-13 while deciding what the docs site could republish.** Running `auditHtmlSource` (all `ALL_RULES` + `DOCUMENT_RULES`) over the 84 `registry/{primitives,recipes,patterns}/**/<name>.html` fragments yields **365 findings**, including criticals: `required-slot` ×80, `valid-variant` ×35, `required-aria` ×34, `focus-trap` ×23, `controller-loaded` ×16, `close-label` ×11, `orphan-part` ×164, `valid-size`/`aria-describedby`/`token-aware-style` ×3. The registry self-audit only runs the *document* rules over this markup, so nothing catches it today. It matters beyond the docs site: `faqir audit` in a user project scans `ui/**` including the reference pages it copied in, so a fresh `faqir init` + `faqir add crud-table` reports a wall of findings from Faqir's own markup. Triage each class — some are intentional partial demos (a card showing only its header trips `required-slot`; a dialog demoed without a controller trips `focus-trap`/`controller-loaded`), some look like real defects (`crud-table` uses `data-variant="primary"` on a part whose variant values are `compact|default|comfortable`; `orphan-part` names slots like `nav-icon`/`nav-label` that no manifest declares). Then decide the contract: complete the references, declare the missing slots, or scope the per-component rules out of reference pages (and teach `faqir audit` to skip them in projects). **Resolved (2026-08-04, 0.9-04):** the sweep found **367** findings, not 365, and the triage split them four ways rather than the three this row imagined — because most of them were not defects at all. **190 were the audit misreading composition.** `data-part` was attributed to the *nearest* enclosing `data-ui`, so `auth-form`, which nests a `card` and puts its own `<form data-part="form">` inside it, had its parts read as the card's (`orphan-part`) and was then reported as missing them (`required-slot`) — one authoring shape, two findings, neither of them wrong markup. Attribution is now *nearest ancestor that **declares** the slot* (`extractComponents(source, file, declaresSlot)`), and `required-slot` asks a different question from the other rules on purpose: `parts` is "whose is it" (nearest declaring owner — which is what keeps a card's `header` the card's, so `dashboard-shell` does not demand `role="banner"` on every card in the page) while `filledSlots` is "is it filled" (anywhere in the subtree). **39 were rules reporting correct markup**, each a real bug now fixed with a test: `valid-variant` judged a nested `<button data-ui="button" data-part="submit" data-variant="primary">` against the *parent's* vocabulary (auth-form's `login|register|forgot-password`); `required-aria` lowercased the value it demanded, turning pagination's declared `aria-label="Pagination"` into a demand for "pagination", and read *conditional* prose ("aria-busy … when in loading state", "role=radiogroup … in single mode") as unconditional; `close-label` demanded `aria-label` on eleven buttons that read "Cancel"/"Close", where adding one would *override* the visible text and break voice control — it now asks for an accessible **name**; and `controller-loaded` was satisfied by the `<!-- @ui:controller dialog.js -->` comment every fragment opens with, while not recognising `faqir-core.min.js`, the file the CDN preamble actually loads. **39 were the scoping question this row asks**, and the answer is content-derived, never a path list: `controller-loaded` and `focus-trap` assert a runtime in the same file, which a *fragment* by construction never carries, so they are scoped to full documents via the `isFullDocument` the document rules already used — and the copy-for-agents payloads, which are real pages with the CDN preamble, are now asserted clean under the **full** rule set, so the exemption is a scope and not a hole. `focus-trap` also stopped being unsatisfiable: it now survives only where `controller-loaded` also fires, which is the condition its own message names. The remaining ~99 were genuine and were fixed: ~60 styled-but-undeclared parts declared in their manifests (`switch`'s `thumb`, `select-custom`'s `chevron`, eight on `dashboard-shell`, five on `crud-table`, three on `command-palette`), `table`'s two ad-hoc boolean variant groups normalised to the one group its CSS actually selects, `text`'s `2xl|3xl|4xl` declared (the CSS had them), seven `data-variant="heading"` on `text` that no rule paints, a `data-part="control"` that field-group calls `input`, two dismissible `badge`s that are `chip`s, a `role="navigation"`, an `aria-describedby`, a 360px inline knob, and seven `card`s missing the `body` the card contract requires. Nine `required: true` flags that overstated the contract were relaxed with a reason each (`dialog.trigger` — a dialog is opened by whatever the application decides, which is also 0.9-05's question answered; `toast.message`/`close` — required of a toast, not of the empty region that holds them; `crud-table`'s four optional affordances; `settings-page.tabs`, absent from four of five reference layouts), and `required-slot` now skips a component announcing a declared **non-default** `data-state` — `data-state="empty"` is the state in which a table deliberately has no rows. Enforced in three places so it cannot rot: `audit:registry` gate 7 (the full rule set over all 87 fragments), `tests/audit/reference-contract.test.ts` (in `bun test`, plus the fragment/page scoping proven in both directions), and `tests/generator/docs-agents.test.ts` (the payloads under the full rule set). The contract itself is written down in `FAQIR-SPEC.md` §9, not only here. | 0.7-13 | ✅ |
...
| 0.8-13 | **The theme-preview frame's `<main>` is still an inline layout escape — the second of the two sites 0.8-06 was pointed at, and the one that is not a measure problem.** `src/generator/docs.ts` emits `<main style="padding: var(--space-4); display: grid; gap: var(--space-4);">` for each of the twelve theme-preview frames. 0.8-06 retired the *measure* escape it was named beside (the 72rem prose width, 94 pages, now `container`), but this one is a padding + grid escape: its honest replacement is the `grid` primitive (0.8-04) or a `stack`, plus a gutter — `container` alone cannot express it, and `container`'s gutter is `padding-inline` only, so a naive swap would silently drop the frames' block padding. Doing it moves twelve visual baselines, which is why it was filed rather than absorbed into a task whose tests are all about `max-width`. Fix: pick the right primitive for the two concerns (rhythm and inset), update the frames, and re-baseline the theme-gallery screenshots in the same change — natural to fold into 0.8-11's viewport-axis visual pass, which is re-baselining anyway. **Resolved (2026-08-04, 0.9-03):** the frame's `<main>` is now `<main data-ui="surface" data-variant="flat" data-size="md" data-gap="4">`. `surface` is the primitive that pads on *both* axes — `data-size="md"` resolves to `padding: var(--space-4)`, the exact declaration the inline style carried — which is the half `container` could not express and the reason this row was filed rather than absorbed into 0.8-06. The rhythm half needed no primitive at all in the end: 0.9-02 made `surface` a flow root, so §20 spaces the four demos by default and `data-gap="4"` tunes that to the same `--space-4` the `display: grid; gap:` pair was buying. `flat` because the frame *is* the page, not a card on it. The re-baseline turned out to be a smaller claim than this row assumed: **no screenshot baseline covers these twelve frames** — `tests/visual/matrix.ts` builds its documents from registry fragments, not from the docs generator, and `frames/**` is deliberately excluded from the layout-lint sweep (a frame is sized by its parent `<iframe>`, so a viewport-relative gutter would measure the harness). What does gate them is unchanged and green: `tests/a11y/docs-site.pw.ts` axe-scans every frame, `tests/visual/docs-switcher.pw.ts` drives them in a browser, and `tests/generator/docs-themes.test.ts` asserts their contents. 0.9-03 also closed the general case this row was one instance of: `docs-site.test.ts` now fails on **any** `style=` attribute on any page the generator authors, and the other three escapes (two token-swatch sites, the playground preview frame) went with it — swatch colour is now a generated `[data-docs-swatch="<token>"]` rule per token, derived from the same token reference the pages are. | 0.8-06 | ✅ |
| 0.8-14 | **Six committed visual baselines are already stale on `main` — found by 0.8-08 while proving its own changes moved nothing.** Running `bun run test:visual -g "(table|input|stepper)__default"` fails `primitive__input__default__dark__{ltr,rtl}` (2677 px, ratio 0.01) and all four `recipe__table__default__{light,dark}__{ltr,rtl}`, the latter with a **page-height mismatch** — the baseline is 1280×1449, the render is 1280×3042, i.e. the table reference page roughly doubled in length since it was last captured (0.4-22/0.5-x era markup growth) and nobody re-baselined. Confirmed pre-existing, not caused by 0.8-08: `git stash` to a clean tree reproduces the identical six failures. It also means the visual gate gives NO real coverage on those six today — a genuine regression in `table` or `input` would be indistinguishable from the existing noise. Note the matrix is only baselined for the `default` theme; every other theme×scheme cell reports "a snapshot doesn't exist", so `bun run test:visual` has never been green as a whole. Fix: re-capture the six, decide whether the non-default theme cells are meant to be committed or filtered out of the run, and make the suite pass on a clean tree so it can gate again — natural to fold into 0.8-11, which re-baselines for the viewport axis anyway. **Resolved (2026-07-28, 0.9-01):** the whole set was regenerated after that task deleted the harness's `body { padding: 24px }` (every baseline moved anyway), and `bun run test:visual` is now green as a whole — **4 243 passed**, the six named captures among them. The "non-default cells" half turned out not to be a filtering question at all: `tests/visual/__screenshots__/` is **git-ignored** (the §12.2 CI-cache strategy — `main` seeds the cache, PRs restore and diff), the run has always been the full cross-product, and the missing cells were a *partial local* set left behind by an earlier `--grep`-scoped `--update-snapshots`. One unscoped regeneration writes all 4 214 captures (≈164 MB, ~7 min); nothing is excluded on purpose, and `tests/visual/README.md` now says so where the symptom appears. | 0.8-08 | ✅ |
| 0.8-15 | **Attribute-level declarations are now enforced, but VALUE-level ones are not — measured while shipping 0.8-10's `undeclared-attribute`.** That rule asks "is this attribute declared?"; nothing asks "is this *value* declared?", and a sweep of the registry finds **21 selected values across 4 components that their manifest's variant group does not list**: `text` selects `data-size="2xl|3xl|4xl"` and `data-variant="destructive"` (four sizes and a colour the docs, skill and bindings therefore never offer), `table` selects `data-variant="striped|bordered"` against a group whose values are `true|false`, `toast` selects `data-variant="success|error|warning"` against a group that is really a *position* axis (`top-right`…), and `document` selects `data-format="number|currency|percent"` on a cell against the page-size group `a4|letter`. The last three are the interesting half and the reason this is its own task rather than an hour's remediation: they are not typos but *two different axes sharing one attribute* inside one stylesheet, so the fix is a modelling decision per component (split the group, rename the attribute, or declare the second axis) before any rule can be turned on. Ship `undeclared-value` the same way 0.8-10 shipped its pair — pure function in `src/audit/css-rules.ts`, wired into the checker, gate 5 and the browser bundle, with parity fixtures — and take the registry to zero. `tests/primitives/stack.test.ts` already enforces it for one component ("selects on no value a variant group does not declare"), which is the shape to generalize. | 0.8-10 | ⬜ |
| 0.8-16 | **A `data-ui="table"` root is a scroll container at every width, but only ONE page in the registry gives it keyboard access — found by 0.8-11's mobile axe sweep the first time it ran.** `registry/recipes/table/table.css` opens with `[data-ui="table"] { width: 100%; overflow-x: auto; }`, so the recipe — not the page author — is what creates the scrollable region. Whether it *scrolls* depends on the viewport: at 1280 px a five-column table fits and axe is silent; at 390 px it overflows and `scrollable-region-focusable` (WCAG 2.1 A, serious) fires because the region is unreachable by keyboard. 0.8-11 fixed the one page its gate covers (`stats-dashboard`, both table roots, `tabindex="0"`), but the gate only scans the layout-bearing set, so the rest of the registry is untested at 390 px and unfixed: **`table.html` itself carries twelve table roots, `crud-table` three and `document` one — sixteen in all, and not one has `tabindex`**. `crud-table` and `document` pass the mobile sweep today only because their content happens to fit at 390 px — content luck, not a guarantee, and a column added to either turns them red. Fix: decide where the obligation lives (the recipe's documented anatomy, so every consumer inherits it — probably `tabindex="0"` plus `role="region"` + `aria-labelledby` where a heading exists, which is the WAI pattern and the reason this is a modelling decision rather than a sed), apply it across `table.html` and every consumer, and add the assertion to the table recipe's own suite the way `tests/patterns/stats-dashboard.test.ts` now has it. Note `tabindex` is pixel-neutral (0.8-11 re-verified three captures byte-identical after adding it), so no visual baseline moves; the docs site and `registry-index.json` do need regenerating. Consider also whether the mobile axe sweep should widen from the layout-bearing set to any page containing a scroll container — the narrower question 0.8-11 deliberately did not answer. **Two more of the sixteen came due in 0.9-01, and the "content luck" prediction was exactly right.** Deleting the visual/a11y harness's `body { padding: 24px }` and its `main.vr-root` flex column removed the `align-items: flex-start` that had been shrink-wrapping every fragment to its intrinsic width; as block children of a bare `<main>` the scroll containers finally *scroll*, and the 390 px sweep went from green to **8 failures**: `document`'s single table root, plus a case this row had not counted — `grid`'s `[data-scroll]` snap strip (`grid.css` gives it `overflow-x: auto` below `sm`, so the component creates the region exactly as `table` does). Both fixed at the source with `tabindex="0"` (pixel-neutral, as recorded here; `registry-index.json` and the docs site regenerated), so the remainder is **fifteen table roots — twelve in `table.html`, three in `crud-table` — plus whatever else a widened sweep finds**. The modelling decision is untouched and still belongs here: this was a spot fix of what a gate proved broken, not the contract change. | 0.8-11 | ⬜ |
...
| 1.0R-13 | **Seven `data-ui` values ship with no manifest and no documentation — found by 1.0R-11 the first time the `unknown-component` rule was pointed at Faqir's own registry.** `button.css` declares `[data-ui="button-group"]`, `button.html` demonstrates it, and no `button-group` component exists; the same is true of `input-group` (input.css), `radio-group` and `radio-label` (radio.css), `checkbox-label` (checkbox.css), `switch-label` (switch.css) and `heading` (text.css), and four of them are used by patterns as well. 1.0R-11 made the rule *derive* them from the stylesheets that define them (`componentDefinedUiValues`) rather than report the framework's own markup as broken — the same resolution 1.0R-10 gave `prose` — but derivation only bought silence: they are in no manifest, so they are in NO manifest-derived surface. An agent reading `SKILL.md`, `llms.txt`, `components/index.html` or the Vue/React bindings cannot discover `button-group`, and `faqir add button` installs a stylesheet styling a name nothing documents. The decision is per value and is a registry-design question, not an audit one: `button-group`, `input-group` and `radio-group` are structural wrappers that look like components that were never given a manifest, while `radio-label`, `checkbox-label`, `switch-label` and `heading` look like companion styling in the `prose` sense. Fix: decide each way, then either write the manifest (and let every derived surface pick it up) or document it as styling the way 1.0R-10 documented the base layer — and gate the outcome, so a new `[data-ui=…]` in a component stylesheet cannot again arrive undocumented. | 1.0R-11 | ⬜ |
| 1.0R-14 | **The documentation site forbids the thing its own engine needs: the generated CSP has no `'unsafe-eval'`, so every live `l-*` demo on the site is dead — found by 1.0-02 while verifying the CSP claim it was writing down.** `renderHeadersFile()` in `src/generator/docs.ts` emits `script-src 'self'` with no `'unsafe-eval'`, and `site/dist/_headers` ships it. The engine compiles every expression with `new Function`, so under that policy it loads, bootstraps, strips `l-cloak` and mounts controllers — and then every expression throws `EvalError` inside `evaluate()`, which catches, warns and returns `undefined`. Measured in real Chromium against `site/dist` served with the committed header: `<span l-text="count">` rendered `""` instead of `1` and the click handler did nothing, while `style-src 'self' 'unsafe-inline'` (already present) correctly kept the cloak rule alive. Five site pages load the engine with live directives on them, `/engine/` foremost. Fix: add `'unsafe-eval'` to the generated `script-src` and assert it in `tests/generator/docs-foundations.test.ts` (which today asserts only `toContain("Content-Security-Policy: default-src 'self'")` and so cannot see this), then decide whether the site wants a second, engine-free policy for the pages that carry no directives. `docs/security.md` §1 is the written form of the requirement. | 1.0-02 | ⬜ |
| 1.0R-15 | **The reactive-engine guide renders none of itself: a nested `-->` closes its source comment early and a stray `<template>` swallows the rest — found by 1.0-02 while loading `/engine/` to check the CSP.** `site/content/engine.html` opens with an HTML comment whose fourth line reads ``The five `<!-- @faqir:engine-* -->` markers are replaced by…`` — the inner `-->` ends the comment there. Lines 5–13 then leak into the page as 301 characters of visible maintainer prose, and line 9's mention of `<template data-docs-example>` is parsed as a real start tag, so the parser puts the entire guide inside an inert template. Confirmed in Chromium: `[data-ui="prose"]` has exactly four children — a text node, an 82-character comment, a 301-character text node and a `TEMPLATE` — and `document.querySelectorAll('[l-data]')` returns **0** on the page whose whole point is live `l-data` examples. 34 KB of generated guide, five derived tables and every live demo are invisible. The byte-level docs tests pass because they audit the file's markup, never its parsed DOM. Fix: escape the marker in the comment (or drop the comment into a form with no inner `-->`), and add a parse-level assertion — a page's rendered DOM must contain the elements its source declares — so a swallowed section cannot ship again. Note this page is also the one 1.0R-14 measures. | 1.0-02 | ⬜ |
| 1.0R-16 | **Three components ship an `rgba()` shadow that `faqir audit` reports as an error in every project that installs them — found by 1.0-03 while auditing a migrated v0.2.4 project.** `registry/primitives/switch/switch.css` (twice), `registry/primitives/stat/stat.css` and `registry/patterns/document/document.css` write `rgba(0, 0, 0, …)` in a `box-shadow`, which the `no-hardcoded-values` rule flags. `audit:registry`'s CSS pass runs var() resolution, `undeclared-attribute` and `breakpoint-canon` — not the rule set the audit applies to user projects — so the registry is not held to a rule it enforces on everyone else, and no project that installs `switch` can reach a clean audit through no fault of its own. Two halves: pick the fix (nearest existing token changes the alpha 0.1 → 0.04 and moves pixels; a new shadow token does not, but tokens are a documented surface), and add the CSS rule set to the registry gate so the class cannot come back. `tests/migration/upgrade-v024.test.ts` asserts the residual findings are exactly these, so it will fail — informatively — when they are fixed. | 1.0-03 | ⬜ |
| 1.0R-17 | **The `landmark` rule flags any `role="dialog"` inside `<main>`, so a date-picker in a form is a warning on every page that follows the framework's own advice — found by 1.0-03 while auditing a migrated v0.2.4 project.** `landmarkRule` in `src/audit/rules.ts` treats `<dialog>`, `role="dialog"`, `role="alertdialog"` and `data-ui="dialog"` alike, and the reasoning it carries — overlays belong outside the content flow — is right for a modal and wrong for a popup anchored to a field it belongs to. The registry's own `date-picker.html` and `popover.html` carry `role="dialog"` on a part nested inside their component root; they escape the rule only because a reference fragment is not a page. Decide the distinction (a modal is `aria-modal="true"`; an anchored popup is not) and pin both directions. | 1.0-03 | ⬜ |
| 1.0R-18 | **`faqir add empty-state` in a project that installed the *pattern* resolves the *primitive*, backfills the wrong baseline and would upgrade into a directory the project does not use — found by 1.0-03 while building the v0.2.4 fixture project.** `findComponentInRegistry` searches primitives first and returns on the first hit, while `findInstalledLayer` reads the layer the config records; `faqir upgrade` uses the first for `installedDir` and the second for the report, so for the one name that exists in two layers they disagree. The fixture project drops `patterns/empty-state` for exactly this reason. Blocked on the same decision as 0.9-13 (one name, two components) — fix that first, then make both resolvers take the installed layer into account. | 1.0-03 | ⬜ |
